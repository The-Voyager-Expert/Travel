...
- ⚠️ **New main page = add to the Main Pages index, same pass.** Any page added to the toolbar `ITEMS` array in `Travel-Website/assets/toolbar.js` must also be added to `Travel-Website/Website-Main-Pages-Links.html` — the Main Pages hub shared with friends — as a leaf with its live link. (`Travel-Website/index.html` is a redirect to Guides-Index.html, not the hub.) Toolbar and the Main Pages index must list the same set (Guides index is the one deliberate omission). Full rule: `Brain/Reference/Toolbar.html` § 4.
- ⚠️ **Never commit `_build/` — it breaks the live deploy.** Guide `_build/` dirs (full-size source photos + verification logs) are research scaffolding, NOT site content. GitHub Pages publishes the **entire** repo tree on every build and enforces a **~1 GB total-site limit** — splitting into smaller pushes does NOT help, because each build still publishes the whole site. By 2026-06-20 committed `_build/` dirs were 313 MB across 2150 files, pushing the site to 1032 MB and failing every deploy with `"Page build failed"` / `duration 0` (looks exactly like an infra flake but is NOT transient — it fails on every commit). `_build/` is gitignored (`_build/` + `**/_build/`); the cribs read it from the shared Drive, not git, so untracking costs them nothing. If `git ls-files | grep '/_build/'` ever returns >0, run `git rm -r --cached -f -- '*/_build/*'` (keeps disk files). Check size anytime: `git ls-tree -r -l origin/main | awk '{s+=$4} END{print s/1048576" MB"}'` — keep it well under 1024. Full record: memory `project-pages-1gb-limit`.
...
- `Travel/On Demand/Weather - On Demand.html`
...
**Operational reference:** `Brain/Reference/Cleanliness Checks.md` — 413-rule enforcer spec (how each validator catches a violation). Every entry cites its CORE RULES source. Read this when adding or debugging a validator check; use `Brain/Reference/Separation Map.md` to find the authoritative CORE RULES file for any topic.
...
| `build_search_index.py` | Search index — regenerates `Travel-Website/assets/search_index.json` from all shipped guides (stops, attractions, tours, restaurants, section names). Auto-runs as the last step of `guide_tools.py ship`; run manually after editing a guide without shipping |
