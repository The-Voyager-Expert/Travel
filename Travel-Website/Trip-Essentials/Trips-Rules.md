...
- **No `+1` next-day shorthand on overnight flights.** Per owner 2026-06-30 (*"i dont like this Aug 14 arrive 8:25 AM +1. i get confused"*): when a flight lands the day after it departs, do NOT append `+1` to the arrival time in the row. Spell out the arrival date in prose, inline, in the same row's last cell: `arrive HH:MM AM/PM on Month Dayth` (per owner 2026-06-30, final form: *"Aug 14 5:25 PM ATL → MUC DL130 arrive 8:25 AM on August 15th"*), with the `on Month Dayth` portion wrapped in `<span class="red">` (per owner 2026-06-30: *"on August 15th make it red"*) so the next-day date pops visually. Example:
  ```html
  <div class="ftable"><span class="fc">Aug 14</span><span class="fc">5:25 PM</span><span class="fc">ATL → MUC</span><span class="fc">DL130</span><span class="fc">arrive 8:25 AM <span class="red">on August 15th</span></span></div>
  ```
  No separate sub-line — everything stays in the one row. (Earlier iterations tried a muted sub-line and then a same-size `.fc` sub-line below the row; both were rejected in favor of this single-row inline form — don't reintroduce either.) Applies to any overnight leg, not just transatlantic. Same-day arrivals keep the plain `arrive HH:MM` / `land HH:MM` in the row with no date suffix.
...
## Trip header color (updated 2026-07-26)

`.trip-header` uses a warm peach/sand background (`#f0e6d6`) with a matching border (`#e0d2be`) — NOT `var(--surface2)`. This distinguishes the trip header from the airport address block (`.airport-addrs`), which uses `var(--surface2)`. Without this, both surfaces look identical and it's hard to tell where one trip ends and the next begins.

Do not change back to `var(--surface2)` or any blue-toned color (blue conflicts with airport codes and address links).
...
**Show the traveler's actual attendance dates, not the full multi-track event span.** Per owner 2026-07-01 (*"he is on the 26 to 29"*, correcting an initial add that used FLoC's full published range of Jul 20–23 & 26–29): a large federated/multi-track event may run longer than any one person attends. Default to the full official range only if no narrower attendance info exists (e.g. from the flight dates bracketing a specific sub-range, or an explicit correction like this one) — once given the actual attendance window, use that, not the umbrella event dates.

Format: `City <span class="hotel-sep">·</span> Date range · conference dates`. Example:
```html
<div class="hotel-meta">Lisbon <span class="hotel-sep">·</span> Jul 26–29 · conference dates</div>
```
