...
- **No `+1` next-day shorthand on overnight flights.** Per owner 2026-06-30 (*"i dont like this Aug 14 arrive 8:25 AM +1. i get confused"*): when a flight lands the day after it departs, do NOT append `+1` to the arrival time in the row. Instead, drop a line directly below the `.ftable` row spelling out the arrival date, formatted `[Month Day] Arrival` (per owner 2026-06-30: *"to look better change to adding a line like that Aug 15 Arrival"*), styled to match the flight row — `.fc` class, same 13px/`var(--text)` look, NOT smaller/muted (per owner 2026-06-30: *"same size and look"*). Example:
  ```html
  ...
  <div class="fc" style="padding:1px 0;">Aug 15 Arrival</div>
  ```
