...
**One uniform card format on all 29 pages — no exceptions, no tiers.** Every card on every page — including Safari-Guide, Wonders-of-the-World, US-National-Parks, and National-Parks-World — is exactly `.best-of-name` (plain text) / `.best-of-tag` / `.best-of-meta` / `.best-of-links` (a row of `.best-of-link` pills — Guide + Wikipedia + official/booking resources as available). These are all the same type and level of guide; the only reason 4 of them aren't named `Best-*.html` is legacy filenames. No descriptions, no name-anchor wrappers, no inline `style=`, no `.best-of-desc`. All 29 pages share identical `.best-of-section-label`, `.best-of-card`, `.best-of-name`, `.best-of-tag`, `.best-of-meta`, `.best-of-links`, `.best-of-link`.
