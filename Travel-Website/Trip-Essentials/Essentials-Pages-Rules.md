...
All 29 pages in "The Best of" sidebar section share ONE CSS block, living once in `Travel-Website/assets/web-travel-style.css` (consolidated out of 29 duplicated inline `<style>` blocks 2026-07-05, per the SHARED CSS ONLY rule — see § Active/selected pill colour below). No page has its own inline `<style>` block for these classes — `check_best_of_css_standard` hard-fails if one reappears. This scope covers the 25 `Best-*.html` pages plus 4 more Trip-Essentials pages that share "The Best of" sidebar section on Guides-Index.html but aren't named `Best-*.html`: **Safari-Guide.html, Wonders-of-the-World.html, US-National-Parks.html, National-Parks-World.html** — 29 pages total, one shared CSS block, one visual standard.
...
**Per-card links on all 29 pages.** Every card carries `.best-of-name` / `.best-of-tag` / `.best-of-meta` **plus a `.best-of-links` block of `.best-of-link` pills** (Guide + Wikipedia + official/booking resources as available). Links were restored across the 25 `Best-*.html` pages 2026-07-05 — the earlier "minimal, name/tag/meta only" format was a miscommunication. The 4 extended pages (Safari-Guide, Wonders-of-the-World, US-National-Parks, National-Parks-World) additionally keep **per-card descriptions** (`.best-of-desc`). All 29 pages share identical `.best-of-section-label`, `.best-of-card`, `.best-of-name`, `.best-of-tag`, `.best-of-meta`, `.best-of-links`, `.best-of-link` values — only the extended 4 are permitted the extra `.best-of-desc`.
...
**Allowed card classes.** A card on any of the 29 pages contains ONLY `.best-of-name`, `.best-of-tag`, `.best-of-meta`, and a `.best-of-links` block of `.best-of-link` pills — directly inside `.best-of-card`, no wrapper div around name/tag. No emoji icon spans, no `best-of-card-head` wrapper, no top-of-page "resources" grids. `check_best_of_css_standard` hard-fails if a card contains any class outside those allowed. Guide pills inside `.best-of-links` must point at a guide file that exists (version-drifted links like `rome_v2.html` when only `rome_v1.html` exists are dropped/repointed on restore).

**The 4 extended pages allow one more class.** `.best-of-desc` (per-card description paragraph) is permitted on Safari-Guide, Wonders-of-the-World, US-National-Parks, and National-Parks-World only. Icons and card-head wrapper divs are still forbidden everywhere.

---
*Added 2026-05-29. Currency Guide section added 2026-06-14. Banner + pill colour rules added 2026-06-19. Title-banner LOCKED spec, margins, filenames + new-page checklist added 2026-06-21 (banner h1 size/colour/weight, emoji, body/`.wrap` gutter, and filename-space checks now enforced by `brain_check`). Pickleball bar color locked yellow 2026-06-22. `data-guide` attribute convention + guide HTML transit-card comment documented 2026-06-22. Best-of pages renamed `sf-` → `best-of-` prefix and locked to minimal card content 2026-07-04. Extended to 4 more sidebar pages (Safari Guide, Wonders of the World, US National Parks, National Parks by Country) 2026-07-05 — same visual standard, descriptions/links kept, icons removed. CSS consolidated from 29 duplicated inline blocks into one shared block in `web-travel-style.css` 2026-07-05. Per-card links (`.best-of-links`/`.best-of-link`) restored across all 25 `Best-*.html` pages 2026-07-05 — the prior "minimal, name/tag/meta only" lock was a miscommunication; validator updated to allow links on all 29 pages (descriptions still extended-4 only).*
