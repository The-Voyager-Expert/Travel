...
`Currency-Guide.html` is generated by `Brain/scripts/build/build_currency.py` (static country→currency map in-script + live USD rates) and enforced by `Brain/scripts/validate/validate_currency.py`. **Never hand-edit the HTML** — change the script and rebuild. The validator fails on any drift from the rules below.
...
Refresh: monthly (and on demand) via `python3 Brain/scripts/build/build_currency.py && python3 Brain/scripts/validate/validate_currency.py`.
