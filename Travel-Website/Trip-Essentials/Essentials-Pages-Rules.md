...
**Card summary** (`.best-of-desc`) — **MANDATORY, one per place card, never removed:** 13px / `var(--text)` / `line-height: 1.5` — a factual couple of lines saying what the place is and why it's notable. Plain prose: no emoji, no money symbols, no invented figures. Sits **between `.best-of-meta` and `.best-of-links`** — it carries `margin-top: auto` (with `.best-of-desc + .best-of-links { margin-top: 0 }`), so the summary + link row are pinned to the bottom of the card and the summary reads directly above the links. When a new place card is added to any Best-of page, it MUST include a summary — a card without one hard-fails `check_best_of_css_standard`.
