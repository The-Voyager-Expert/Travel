...
**One uniform card format on all 29 pages — no exceptions, no tiers.** Every place card on every page is exactly `.best-of-name` (plain text) / `.best-of-tag` / `.best-of-desc` (a short 2-line summary of the place) / `.best-of-meta` / `.best-of-links` (a row of `.best-of-link` pills — Guide + Wikipedia + official/booking resources as available). All 29 are the same type and level of guide, all named `Best-*.html`. No name-anchor wrappers, no inline `style=`. All 29 pages share identical `.best-of-section-label`, `.best-of-card`, `.best-of-name`, `.best-of-tag`, `.best-of-desc`, `.best-of-meta`, `.best-of-links`, `.best-of-link`. (`.best-of-desc` is optional per card — the `Best-Of-Index.html` category cards, which point at other pages rather than a place, carry none.)
