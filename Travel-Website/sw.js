...
/* 2026-08-08: Free-entry row — 🆓 retired fleet-wide (owner rule 2026-08-08). The row was `<div>🆓 Free</div>`; it is now `<div><span class="free-flag">No ticket</span></div>`, with a torn-ticket-stub icon drawn by .free-flag::before in guide-style.css. Two reasons: 🆓 is Latin text inside a blue-grey iOS badge that ignores CSS color — the same defect that retired the up-right arrow site-wide (CLAUDE.md sixth non-negotiable) — and it was the only non-pictogram in an icon row of pictograms (🏛️ ⏰ 🚫 📍 💵). The mark is a CSS mask, so it takes the guide accent as a background-color and retints to #d4874a in dark mode like every other guide foreground; the artwork lives in exactly one place, so a future swap is a one-line change. Wording moved with it: "No ticket" states the useful fact (nothing to buy, nothing to queue for) where "Free" only said the obvious; timed entry still lives in a 🎟 .ticket-box, never on this row. 505 rows across 153 guides + the guide_tools.py stub. validate_itinerary.py hard-fails on any surviving literal 🆓 and on any .free-flag row whose text is not exactly "No ticket", then normalises the row back to the legacy token in memory so the ~30 downstream position-5a checks keep matching one canonical form. guide-style.css -> v116. CACHE to v316. */
/* 2026-08-08: ICS action row (Export to Calendar / All Stops Map / Preview Optimized / Save for Offline / I’ve Been) goes to font-weight 700. Owner reported it as a different colour from the "~45 min" .stop-dur chip and the "📖 Read more" chip; measured, all three were already byte-identical on all three properties (#f5f0e6 fill / #bba070 border / #5a3c0e ink in light, #222120 / #444 / #c8a060 in dark) because every one of them is built from --c-next-bg / --c-next-border / --c-action-text. The row alone inherited font-weight:normal from .overview-extra-link, and 400 strokes of a colour read lighter than 700 strokes of the same colour — the identical diagnosis behind the Read more chip (v115) and the open-now label (v112). One declaration on #ics-pill-row > .overview-extra-link, so desktop and mobile move together; no stylesheet outside guide-style.css touches this row. The toggled-on states (.tve-been / .tve-saved) are unaffected — they signal "on" with terracotta ink and border, not weight. guide-style.css -> v117. CACHE to v317. */
var CACHE = 'travel-cache-v317';
...
var MIN_VERSIONS = { 'guide-style.css': 117, 'toolbar.js': 205, 'mobile.css': 66, 'web-travel-style.css': 12, 'Read-About.css': 2, 'best-of-features.js': 1 };
