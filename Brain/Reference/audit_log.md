...
- **Trigger:** owner — "audit the reference folder" + "batch reship run."
...
- **Trigger:** owner — "why the status dot keep being missed. this needs to be better implemented in the validator to fail" + "guides are shipped with this problem all the time."
...
- ❓ Open Questions: none active (the CORE RULES checksum-drift question was resolved & closed by owner 2026-06-13).
...
**In-session edits (CORE RULES change, owner-approved):**
...
- **Guide:** The United Arab Emirates (UAE) 2-day guide (united_arab_emirates_v1.html) — country-scoped per Wifey's request; full parenthetical name used in title + index ("use the entire name as i did").
...
- **2 documented validator deviations:** (1) parentheses in title-city — owner explicitly requested the parenthetical name; (2) title-address has no street number — Dubai addresses have none (official address is boulevard + PO Box only). Both surfaced to owner.
...
**Trigger.** owner approved all 5 parked proposals (A–E) in a single pass.
...
**Trigger.** owner: "go on" after the 18-file Reference audit.
...
**Trigger.** owner: "Do a deep [audit] of the files under the reference folder. Audit one file at a time."
...
2. `Rules for Claude.html` § 4 Publishing — stale pointer `Brain/CORE RULES/Toolbar.html § 5` → `Brain/Reference/Toolbar.html § 6` + notes toolbar.js lives at `Guides/toolbar.js` (owner-authorized: "make sure it is clear everywhere" / "fix all the references"). Checksums regenerated (27 hashed, 1 changed).
...
2026-06-02 — Rules for Claude.html § 4 — new rule added: "The Cowork AskUserQuestion tool is never invoked during any Travel task." Approved by owner. Checksums regenerated. brain_check 50/50. Copenhagen interrupted build archived to Travel/archive/Copenhagen_build_interrupted_2026-06-02.
...
**Trigger.** Session continuation: owner reorganized Brain/ folder structure across two sessions. This session completed the pointer update work.
...
**Trigger.** owner: "audit the train icons and rules" + "audit this file Validator Coverage this seems to be very outdated."
...
**Trigger.** owner: "do a deep brain audit skip the guides."
...
4. **`To_Do_List.md` missing ❓ Questions for owner section** — the three-section structure (My Tasks · Rules for Update · Questions for owner) was incomplete; third section absent. **Fixed:** section re-added.
...
**Trigger.** owner deleted `What Goes Into a Guide.html` and `Types of Days and Stops.html` from CORE RULES and created the consolidated `Stops Structure.html` (Phase 1–2). Instruction: "update everything, the entire brain."
...
**Trigger.** owner: "deep audit the brain" — full Brain infrastructure scan per § 9.
...
6. **Marrakech guide — non-standard filename** — `index.html` instead of `marrakech_v1.html`. Skipped by `audit_all_guides.py`. Parked in ❓ Questions for owner.
...
**Trigger.** owner: "run validator against sydney guide" — fix all ❌ failures until 0 remain.
...
**Trigger.** owner: "run validator again lisbon guide" — fix all ❌ failures until 0 remain.
...
**Trigger.** owner: "Fix Sydney tours, look at Iceland for comparison. Same with Lisbon. The background of both has a double color. If CSS template is wrong fix it."
...
**Trigger.** owner: "Rebuild the Lisbon guide under the new rules. All the tours need to be stripped of their stops."
...
**Trigger.** owner: "Audit the validator, do a deep dive, do dont stop until you dont find anything else to fix. then do twice more." Three full passes over both scripts.
...
**Outstanding (parked in To Do List — require owner permission):**
...
- `Rules for Claude.html` §11: "Wifey's solo events" → neutral phrasing (E12 true positive). Also `class="footnote"` at bottom → `class="footer"` (`.footnote` not in canonical CSS).
...
**Trigger.** owner: "the problem is not the past ones is the future ones — we usually fix the past ones and all the new ones keep coming with old format." Full audit of every template Claude reads when building new guides.
...
- `Rules for Claude.html` — ❌ E12: 2 personal name references ("owner only", "not owner")
...
1. Line 177: "performed by owner only" → "not a Claude action" (neutral phrasing — E12 fix)
...
5. Line 455: "maintained by Claude, not owner" → "maintained by Claude" (neutral phrasing — name-check fix)
...
**Context note:** owner confirmed that 2 rules files were deleted intentionally this session (not a Brain error). No checksum failures surfaced — files were either not yet tracked or already removed before checksums were last generated.

### 2026-05-26 — Stops Structure.html fix + Icon Order and Format.html protection

**Trigger.** owner: Stops Structure.html was created incorrectly; Icon Order and Format.html has a special format that must be preserved.
...
**Trigger.** owner: "do a deep audit of the travel folder."
...
**Follow-up fix pass (2026-06-05, per owner "fix it all now") — all flagged items resolved:**
...
- Remaining `guide_v2.css` mentions across the workspace are intentional history (dated changelog/audit entries, retirement notes, archive snapshots) or the two retained CSS files themselves — kept per Wifey's instruction.
...
## 2026-06-06 — doc_workshop E15/E16 cleanup applied (owner-approved "fix it")
...
2026-06-06 — Florence v1 · validated 2026-06-06 14:45 · ✅ 748 passed / 0 failed (4 soft warnings: Day 2 three stops with day-count sentinel; Cappuccino 4 of 5 + Tours 3/3/3 per-platform, both with low-count comments per owner 2026-05-24). New 3-day guide: Hotel Spadai anchor (hotel research playbook — Booking.com 9.5/1535), 11 stops across 3 self-guided days, 11 Wikimedia photos downloaded local, 9 tours (banned-type cull applied: food/tasting-tour names excluded per validator banned list), Yelp review links throughout, all walk/ride times pulled live from Google Maps driving/walking mode via Chrome. Heads Up (5 entries) added to Brain.md Part 3. Index card #57 + Edinburgh/Geneva chain rewired + Europe Map pin added.
...
## 2026-06-07 — Continent-map pin audit + Europe Map country-aware labels (owner-reported: "Marktoberdorf is not in Austria")
...
- **Trigger:** owner — guides moved one level deeper to `Travel-Website/Guides/` and Trip Essentials to `Travel-Website/Trip-Essentials/`; audit the guides for anything the move broke.
...
- **Finding 2 — pre-existing content bugs in United Arab Emirates guide (NOT reorg-caused; REPORTED, not auto-fixed):** 2 hard-fails the validator already catches — (a) `.title-city` "THE UNITED ARAB EMIRATES (UAE)" + the title-case form use parentheses, banned (· separators only); (b) `.title-address` "Sheikh Mohammed bin Rashid Boulevard · Downtown Dubai" has no street number (Hotel Banner §1 requires street name + number). Left for owner — the parens fix is editorial and the address may be a genuine no-street-number Dubai case; not touched during a reorg-scope audit.
