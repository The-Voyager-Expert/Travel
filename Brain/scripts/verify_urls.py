...
BOT_BLOCKED_HOSTS = {
    ...
    "noon.com", "food.noon.com",                   # Noon Food — "Access Denied" bot wall to automated client; site live, major UAE food-delivery platform
...
def fetch(url: str, is_image: bool = False) -> dict:
    ...
    while attempts < max_attempts:
        ...
        try:
            ...
            if r.status_code in (403, 406, 410, 429, 500) and host_of(url) in BOT_BLOCKED_HOSTS:
...
def classify(result: dict) -> str:
    ...
    if status is None:
        return "fail"
    # Known bot-blocker 403/406/410/429/500 → warn (visual verification needed)
    if status in (403, 406, 410, 429, 500) and result["bot_blocked"]:
        return "warn"
...
def _check_cache_regressions(results: list[dict], prev_cache: dict) -> list[str]:
    ...
    for r in results:
        ...
        if prev_status == 200 and cur_status and cur_status >= 400 and not r.get("bot_blocked"):
            warnings.append(
