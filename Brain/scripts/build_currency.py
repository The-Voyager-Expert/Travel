...
Output: Travel-Website/Trip-Essentials/Currency/Currency Guide.html
"""
import argparse, json, os, sys, urllib.request, datetime, html
...
COUNTRIES = [
 ...
 ("Germany","Europe","Euro","€","EUR","EUR","", ["Berlin","Marktoberdorf","Munich","Stuttgart"]),
 ("Greece","Europe","Euro","€","EUR","EUR","", ["Athens","Corfu"]),
 ...
 ("United States","North America","US Dollar","$","USD","USD","", ["Alaska","Atlanta","Austin","Bend","Big Island","Boston","Chicago","Kauai","Los Angeles","Maui","Miami","New York","Oahu","Orlando","Palm Desert","Palo Alto","Pasadena","San Diego","San Francisco","Scottsdale","Seattle","Virgin Islands"]),
 ("Canada","North America","Canadian Dollar","C$","CAD","CAD","", ["Montréal","Québec City","Toronto","Vancouver"]),
 ...
 ("Brazil","South America","Brazilian Real","R$","BRL","BRL","", ["Rio de Janeiro"]),
...
def get_rates(path):
    ...
    if data.get("result") != "success":
        sys.exit("rate fetch failed: " + str(data.get("result")))
...
<style>
/* Typography follows the site scale in _travel_style.css:
...
body {{ font-family: var(--font); background: var(--bg,#f5f4f0); color: var(--text,#1a1917);
  font-size: var(--fs-body,14px); line-height: 1.5; padding-left: 20px !important; padding-right: 20px !important; }}
.index-section {{ max-width: 940px; margin: 8px auto 0; padding: 0 24px; }}
/* last-update stamp uses shared .updated-stamp from _travel_style.css */
...
.content-wrap {{ max-width: 940px; margin: 0 auto; padding: 0 24px; }}
...
<div class="index-section">
  ...
  <input id="cur-search" type="search" placeholder="🔍" autocomplete="off" spellcheck="false">
