...
# ─── Log staleness thresholds (RETIRED 2026-06-21) ───────────────────────────
# A logged PASS no longer expires by age — Links.html § 3 governs ("a PASS never
# goes stale"). The 30-day WARN / 90-day FAIL tiers were removed from the
# verdict logic below to match the CORE RULE (and the 2026-06-17 Dani-approved
# removal of the identical check in validate_itinerary.py). Constants kept here,
# unused, only as a marker of the retired behavior — do not re-wire them.
LOG_STALE_WARN_DAYS = 30  # RETIRED — no longer enforced
LOG_STALE_FAIL_DAYS = 90  # RETIRED — no longer enforced
...
def _strip_diacritics(text: str) -> str:
    """Remove combining marks so 'Mỹ Sơn' → 'My Son', 'Hội An' → 'Hoi An'."""
    return "".join(
...
def tokenize(text: str) -> set[str]:
    """Lowercase + strip diacritics + strip punctuation + drop stop-words."""
...
def log_entry_state(entry: dict, today: date) -> tuple[str, str]:
    ...
    # No age expiry: per Links.html § 3 a logged PASS permanently clears the
    # warning — "a PASS never goes stale." The 90-day FAIL / 30-day WARN tiers
    # were retired from validate_itinerary.py on 2026-06-17 ("Dani-approved;
    # rule wins") but the identical check lingered here as drift — removed
    # 2026-06-21 to match the CORE RULE. The has-entry / is-PASS / valid-date /
    # not-future checks above still hold (a missing or malformed entry fails).
