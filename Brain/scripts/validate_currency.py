...
THEMATIC_GUIDES = {"Best Ski Resorts", "Best Observation Decks", "Safari Guide", "US National Parks", "National Parks by Country"}

def index_cities():
    html = open(INDEX, encoding="utf-8").read()
    return set(re.findall(
        r'dest-flag">[^<]+</span><span class="dest-name">([^<]+)</span>', html)) - THEMATIC_GUIDES
...
def main():
    ...
    for row in countries:
        # Check for duplicates WITHIN this country row only (same city listed twice
        # under the same country is a data error; same city across two countries is
        # intentional — e.g. "Naples" in Italy and "Naples" in United States).
        row_cities: set[str] = set()
        for c in row[7]:
            if c in row_cities:
                dupes.append(f"{c} (duplicate within {row[0]})")
            row_cities.add(c)
            page_cities.add(c)
    idx = index_cities()
    ...
    if not os.path.exists(PAGE):
        ...
        if not m:
            ...
            try:
                ...
                fails.append(f'"Updated" stamp has unrecognised month "{m.group(1)}" — expected English month name')
                mon = 1  # safe default to avoid NameError below
            yr = int(m.group(2))
            page_ym = yr * 12 + mon
        ...
        must_have = {
            ...
            "header separators (.hsep defined)":
                '.hsep {',
            ...
            "rate-main uses terracotta brown (#5C3D11)":
                'font-weight: 700; color: #5C3D11;',
