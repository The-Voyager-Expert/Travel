...
REQUIRED_SECTIONS = [
    ...
    # Behavioral rules, DriftyCat, Quick Reference moved to their authoritative sources
    # (Rules for Claude.html § 3 / § 6, Brain.md Part 1) — 2026-07-21 architectural strip.
]
...
def check_doc_index_vs_core_rules(report: Report) -> None:
    ...
    if block is None:
        # CORE RULES doc index moved from CLAUDE.md to Brain.md Part 1 (2026-07-21).
        # CLAUDE.md is now an entry-point-only file; the full index lives in Brain.md.
        report.ok("Doc-index ↔ CORE RULES integrity (index in Brain.md — skipped).")
        return
...
def check_no_stray_archive_dirs(report: Report) -> None:
    ...
    # The primary vault is TRAVEL_ROOT/archive/. Travel/archive/ is the Google Drive
    # archive folder that predates the migration and persists as a Drive-only mount —
    # it is a SECOND recognized vault, not a stray (added 2026-07-15 after Drive kept
    # re-syncing Travel/archive/ back after rmdir; 422 live Drive files live there).
    vault_primary = (TRAVEL_ROOT / "archive").resolve()
    vault_drive = (TRAVEL_ROOT / "Travel" / "archive").resolve()
    vaults = {vault_primary, vault_drive}
    ...
    for d in TRAVEL_ROOT.rglob("*"):
        try:
            ...
            continue
        if rd in vaults or any(v in rd.parents for v in vaults):
            continue
...
_GUIDE_COMPANION_RE = __import__("re").compile(r'^.+(-read-about|-stops-map)\.html$')
...
#
# Locked mechanism: the .tb-links row is `width:max-content` + `margin:0 auto` +
# `justify-content:flex-start`, so it is exactly as wide as its buttons and centers
# in the VIEWPORT — the same axis the page content centers on — with NO width cap.
# justify-content:flex-start is critical: center overflows on BOTH sides, pushing
# Guides (first item) into negative-scroll territory (regressed + fixed 2026-07-19).
# flex-start ensures overflow always falls off the RIGHT. It scrolls only when the
# buttons exceed the whole window (very narrow screens). Full rule + the why:
...
def check_toolbar_centering(report: Report) -> None:
    ...
    # justify-content:center on .tb-links overflows on BOTH sides, pushing Guides (first
    # item) into negative-scroll territory where it is hidden (regressed + fixed 2026-07-19).
    # Must be flex-start so overflow always falls off the RIGHT.
    if re.search(r"\.tb-links\{[^}]*justify-content:center", flat):
        problems.append(
            "the .tb-links row uses justify-content:center — with no width cap, center "
            "overflows on both sides and pushes Guides (first tab) into negative-scroll "
            "territory where scroll position 0 can never reach it. Use justify-content:flex-start "
            "so overflow always falls off the right end"
        )
...
def _toolbar_item_paths() -> "list[str] | None":
    ...
    for entry in re.finditer(r"href:\s*base\s*\+\s*'([^']+)'(?P<rest>[^}]*)", block):
        ...
        paths.append(unquote(entry.group(1)).split("#")[0])
    return paths
...
def check_toolbar_page_standard(report: "Report") -> None:
    ...
    for entry in re.finditer(r"href:\s*base\s*\+\s*'([^']+)'(?P<rest>[^}]*)", m.group(1)):
        ...
        raw_path = unquote(entry.group(1))
        path = raw_path.split("#")[0]   # strip anchor — anchors point into an existing page
        page = WEB_ROOT / path
        rel = f"Travel-Website/{raw_path}"
        if not page.exists():
            failures.append(
...
def check_banner_title_size(report: "Report") -> None:
    ...
    var(--fs-title). The canonical value is --fs-title: 18px (desktop); mobile.css
    overrides it to 14px. So banners render 18px desktop / 14px mobile — deliberate
    (confirmed 2026-07-15). Pages must use var(--fs-title), never a hardcoded px value.
    The toolbar is universal (check_toolbar_page_standard); guide-page CSS is separate
    (guide-style.css) and out of scope. @media print enlargements are allowed.
    Reference: Brain/Reference/Colors and Font Size.html § Title Banner.
    """
    ...
    # Valid banner title sizes. The canonical is var(--fs-title) (18px desktop,
    # overridden to 14px by mobile.css). A page-level 14px is also accepted —
    # some pages deliberately pin 14px on all breakpoints (e.g. Visas). What is
    # NOT allowed: hardcoded 16px, 18px, 20px, or any value other than these three.
    OK_VALUES = {"var(--fs-title)", "var(--fs-title,18px)", "var(--fs-title, 18px)", "14px"}
    pages = list((WEB_ROOT / "Trip-Essentials").rglob("*.html"))
    for extra in ("Guides-Index.html", "Website-Main-Pages-Links.html", "index.html"):
        p = WEB_ROOT / extra
        if p.exists():
            pages.append(p)
    ...
    for page in pages:
        ...
        for rule in re.finditer(r'([^{}]+)\{([^}]*)\}', html):
            ...
            if not re.search(r'(?:\.header|\.page-header|\.site-header)\s+h1|\.site-title', sel):
                continue
            # font-size — must be the var token, never a hardcoded px value
            m = re.search(r'font-size:\s*([^;]+)', body, re.I)
            if m:
                val = m.group(1).replace("!important", "").strip()
                if val not in OK_VALUES:
                    failures.append(
                        f"{rel}: title-banner h1 font-size is '{val}' — must be "
                        "var(--fs-title) (resolves to 18px desktop / 14px mobile). "
                        "Never hardcode a px value here."
                    )
    ...
    # Shared stylesheet checks. web-travel-style.css must use var(--fs-title) (the
    # token, not a px value). mobile.css intentionally overrides to 14px — that is
    # the deliberate mobile value (desktop = 18px via --fs-title). Both are pinned.
    SHARED_OK = {
        "web-travel-style.css": OK_VALUES,
        "mobile.css": {"14px"},   # mobile override — deliberate 14px, not a token
    }
    for shared_name, ok_set in SHARED_OK.items():
        ...
        for rule in re.finditer(r'([^{}]+)\{([^}]*)\}', css):
            ...
            if not m:
                continue
            val = m.group(1).replace("!important", "").strip()
            if val not in ok_set:
                failures.append(
                    f"assets/{shared_name}: banner h1 font-size is '{val}' — "
                    f"expected {ok_set}. (Desktop: var(--fs-title)=18px; mobile: 14px override.)"
                )
    ...
    if failures:
        ...
        report.ok("banner-title-size — every title banner h1 uses var(--fs-title) (18px desktop / 14px mobile).")
...
def check_css_duplication_summary(report: "Report") -> None:
    ...
    try:
        ...
        mp = re.search(r"Total pages with inline styles:\s*(\d+)", result.stdout)
...
def check_internal_link_space_encoding(report: "Report") -> None:
    ...
    if not WEB_ROOT.exists():
        return
    SKIP_PARTS = {"Guides", "_build", "Recommendations"}
...
def check_search_bar_standard(report: "Report") -> None:
    ...
    # Standard updated 2026-07-15: explicit height:40px; padding changed to 0 18px 0 36px
    #   (vertical padding now controlled by height, left gap for the gold icon).
    #   Accept either the legacy '11px 18px' (no explicit height) or '0 18px 0 36px'
    #   (explicit height:40px form) — both are compliant.
    # Standard updated 2026-07-15: font-size bumped 15px → 16px (iOS auto-zoom fix —
    #   iOS Safari auto-zooms on any focused input whose font-size < 16px).
    # background/color dropped from the strict set (always white; #fff vs #ffffff
    # varies harmlessly across pages).
    _SEARCH_EXPECTED = {
        'width':         '360px',
        'padding':       ('11px 18px', '0 18px 0 36px'),
        'font-size':     '16px',
    ...
    # Pre-load shared CSS variable definitions from web-travel-style.css so that
    # var(--c-search-focus-border) etc. resolve correctly in HTML files that don't
    # inline the shared stylesheet's :root block.
    # Only parse the light-mode top-level :root block — stop before any @media
    # to prevent dark-mode overrides from polluting the expected light-mode values.
    _shared_css_path = web_root / "assets" / "web-travel-style.css"
    _shared_var_defs: dict = {}
    if _shared_css_path.exists():
        try:
            _shared_src = _shared_css_path.read_text(encoding="utf-8", errors="replace")
            _media_idx = _shared_src.find('@media')
            _root_only_src = _shared_src[:_media_idx] if _media_idx >= 0 else _shared_src
            _shared_var_defs = {m.group(1).strip(): m.group(2).strip()
                                for m in _VAR_DEF_RE.finditer(_root_only_src)}
        except Exception:
            pass
    ...
    for path in sorted(html_files):
        ...
        # Seed with shared CSS vars so var(--c-search-focus-border) etc. resolve
        # correctly in HTML files that don't inline the shared stylesheet.
        var_defs = dict(_shared_var_defs)
        var_defs.update({m.group(1).strip(): m.group(2).strip()
            ...
            for prop, expected in _SEARCH_EXPECTED.items():
                ...
                if pm:
                    ...
                    if resolved not in allowed:
                        expected_str = ' or '.join(f"'{v}'" for v in allowed)
                        failures.append(
                            f"{rel}: search bar '{prop}' resolves to '{resolved}', "
                            f"expected {expected_str} — § 14 Colors and Font Size.html."
                        )
...
def check_index_stat_row(report: "Report") -> None:
    ...
    Standard (Brain/Reference/Colors and Font Size.html § 15, updated 2026-07-19):
    one .stat-row directly under the banner holding two small grey text lines —
    #guide-count (total guides) on the left, #country-count on the right. Both are
    plain text (no pills/dots/buttons). JS sets them via updateStats():
      elG.textContent = allCards.length + ' guides'
      elC.textContent = allSections.length + ' countries'
    The All/Been/Want toggle was removed 2026-07-19 — no visited/on-the-list counts.
    """
    import re as _re
    guides_index = WEB_ROOT / "Guides-Index.html"
    if not guides_index.exists():
    ...
    # 4. Left line JS — total guide count, 'guides' label, plain text.
    # Since the 2026-07-19 toggle removal, updateStats() uses elG.textContent (not an IIFE).
    if "elG" not in html or "' guides'" not in html:
        fails.append("left line JS must set elG.textContent with ' guides' label")
    elif _re.search(r"elG[^;]*stat-pill", html):
        fails.append("left line must be plain text — no stat-pill / button styling")

    # 5. Right line JS — total country count, 'countries' label, plain text.
    if "elC" not in html or "' countries'" not in html:
        fails.append("right line JS must set elC.textContent with ' countries' label")
    elif _re.search(r"elC[^;]*stat-pill", html):
    ...
    if fails:
        ...
        report.ok("index stat row — guides left, countries right, per § 15.")
...
def check_guides_index_topbar_layout(report: Report) -> None:
    ...
    spec (Navigation.html § 8, updated 2026-07-19): #search-wrap is full-width
    and centers the search bar (360px). The All/Been/Want status toggle was
    removed in the 2026-07-19 visited-pill overhaul — this check no longer
    looks for #status-toggle.
    """
    index_path = TRAVEL_ROOT / "Travel-Website" / "Guides-Index.html"
    ...
    if re.search(r'<div\b[^>]*id=["\']status-toggle["\'][^>]*>', html_text, re.IGNORECASE):
        report.fail("check_guides_index_topbar_layout: #status-toggle element found in Guides-Index.html "
                    "— the All/Been/Want toggle was removed in the 2026-07-19 overhaul and must not be "
                    "re-added (Navigation.html § 8)")

    # Topbar pill spacing — .view-toggle must use display:contents, never its own gap or margin:auto (Rule 572)
    vt_match = re.search(r'\.view-toggle\s*\{([^}]+)\}', html_text)
    if vt_match:
        vt_block = vt_match.group(1)
        if re.search(r'\bmargin\b[^;]*auto', vt_block):
            report.fail("check_guides_index_topbar_layout: .view-toggle must not use margin:auto — "
                        "use display:contents so children inherit topbar gap:14px uniformly (Rule 572, Navigation.html § 8)")
        if re.search(r'\bgap\s*:', vt_block) and 'contents' not in vt_block:
            report.fail("check_guides_index_topbar_layout: .view-toggle must not define its own gap — "
                        "use display:contents so Trips and Compare inherit topbar gap:14px (Rule 572, Navigation.html § 8)")

    # OWNER-DIRECTED 2026-07-20: ✈️ Trips button must be present in .view-toggle
    # (added by owner, stripped by a crib 2026-07-20 — now hard-gated)
    if not re.search(r'id=["\']btn-my-trips["\']', html_text, re.IGNORECASE):
        report.fail("check_guides_index_topbar_layout: #btn-my-trips (✈️ Trips button) is missing from "
                    "Guides-Index.html — this was an owner-directed change (2026-07-20) and must not be "
                    "removed. It is an <a> inside .view-toggle pointing to Trip-Essentials/Trips.html. "
                    "See Navigation.html § 8 and Cleanliness Checks Rule 570.")

    if not report.failures or not any("check_guides_index_topbar_layout" in f for f in report.failures):
        report.ok("Guides-Index topbar layout — #search-wrap centering + #btn-my-trips present "
                  "(Navigation.html § 8)")
...
def check_cascade_sync(report: Report) -> None:
    ...
    Uses mtime comparison with two grace windows:
    • Script/reference pairs — 7 days.  Scripts (guide_tools, brain_check,
      validate_itinerary) are edited daily; a 1-hour window produced 5 constant
      false-positives every session.  A week of lag before the ref doc is
      considered stale is the practical signal threshold.  (Changed 2026-07-15
      after the audit flagged these as "recurring benign" noise.)
    • CORE RULES catch-all — 1 hour.  CORE RULES edits are deliberate and
      intentional; the dependent doc should follow within the same session.
    ...
    GRACE_SECONDS = 7 * 24 * 3600   # 7 days — scripts are edited constantly
    GRACE_CORE_SECONDS = 3600        # 1 hour — deliberate CORE RULES edits
    ...
    if core_rules_dir.is_dir():
        ...
        if vi.exists() and newest_core > vi.stat().st_mtime + GRACE_CORE_SECONDS:
...
def check_world_map_render_cities_disabled(report: "Report") -> None:
    """HARD-FAIL if renderCities() in World-Map.html does not have an early return.

    Maps-Layout.html §10 invariant: 'Pins are guide-only — no non-guide major world
    city markers or labels (renderCities() is intentionally disabled).'
    The function is disabled via an early `return;` immediately after
    `cityLayer.clearLayers()`. If that return is removed, non-guide world-city markers
    and name labels reappear on the map, violating the guide-only-pins rule.
    Drift class: a crib 'improves' renderCities() by removing the disable comment+return.
    Rule: Maps-Layout.html §10. Cleanliness Checks Rule 577. (added 2026-07-20)
    """
    map_path = TRAVEL_ROOT / "Travel-Website" / "Trip-Essentials" / "Maps" / "World-Map.html"
    if not map_path.exists():
        report.fail("check_world_map_render_cities_disabled: World-Map.html is missing")
        return
    text = map_path.read_text(encoding="utf-8", errors="ignore")
    # The function must have 'cityLayer.clearLayers();' followed (within a few lines)
    # by an unconditional 'return;' before any rendering logic.
    m = re.search(
        r'function renderCities\(\)\s*\{[^}]{0,300}?cityLayer\.clearLayers\(\);[^\n]{0,80}\n[^\n]{0,10}return;',
        text, re.DOTALL,
    )
    if not m:
        report.fail(
            "check_world_map_render_cities_disabled: renderCities() is missing its early "
            "return; — non-guide world-city markers/labels will reappear. "
            "The function must have 'cityLayer.clearLayers(); return;' as its first two "
            "statements to stay disabled. Maps-Layout.html §10 / Cleanliness Checks Rule 577."
        )
    else:
        report.ok("World-Map.html renderCities() — early return present; guide-only pins invariant holds")
...
def check_toolbar_group_icon_consistency(report: "Report") -> None:
    ...
    EXEMPT_GROUPS = {"Flights", "Safety", "Time Zones"}
    ...
    for group_label, children_text in group_blocks:
        ...
        # Parse each child object individually to catch both text: and full: fields.
        # The hamburger renders ch.full || ch.text, so we check BOTH values:
        #   • text: must start with the group icon (desktop chip label)
        #   • full: (if present) must also start with the group icon (hamburger effective label)
        CHILD_OBJ_RE = _re.compile(r'\{[^{}]+\}', _re.DOTALL)
        for child_m in CHILD_OBJ_RE.finditer(children_text):
            obj = child_m.group(0)
            text_m = _re.search(r"text:\s*'([^']+)'", obj)
            full_m = _re.search(r"full:\s*'([^']+)'", obj)
            text_val = text_m.group(1) if text_m else ''
            full_val = full_m.group(1) if full_m else ''
            if text_val and not text_val.startswith(group_icon):
                failures.append(
                    f"'{group_label}' child text: must start with '{group_icon}' — got: '{text_val}'"
                )
            if full_val and not full_val.startswith(group_icon):
                failures.append(
                    f"'{group_label}' child full: must start with '{group_icon}' — got: '{full_val}'"
                    " (hamburger uses full: when present)"
                )
    ...
    if failures:
        ...
        report.ok("toolbar.js dropdown groups — all non-exempt children use uniform group icon (text: and full:)")


def check_toolbar_child_label_text(report: "Report") -> None:
    """Fail if any toolbar dropdown group child has a text: field that is emoji-only.

    The hamburger renders ch.full || ch.text (whichever is truthy first).
    Either field can be the problem — so we check both:
      • text: emoji-only (no words) → '📊' alone → icon with no context
      • full: emoji-only (no words) → same in hamburger when full: overrides text:
      • text: text-only (no icon) → 'Stats Across the Caribbean' → no icon prefix
        (NOTE: the icon-prefix direction is gated by check_toolbar_group_icon_consistency;
         this check catches the missing-words direction from either field.)

    Added 2026-07-19 after '📊 Stats Across the Caribbean' appeared as just '📊' in
    the iOS Safari hamburger menu twice in the same week — and also to cover the reverse
    case where a label shows text only with no emoji icon.
    """
    import re as _re

    toolbar_path = TRAVEL_ROOT / "Travel-Website" / "assets" / "toolbar.js"
    if not toolbar_path.exists():
        report.fail("check_toolbar_child_label_text: toolbar.js not found")
        return
    ...
    for group_label, children_text in group_blocks:
        for child_m in CHILD_OBJ_RE.finditer(children_text):
            ...
            text_val = text_m.group(1) if text_m else ''
            full_val = full_m.group(1) if full_m else ''
            # Check text: for word content (always)
            if text_val and not _re.search(r'[a-zA-Z]{3,}', text_val):
                failures.append(
                    f"'{group_label.strip()}' child text: is emoji-only: '{text_val}'"
                )
            # Check full: for word content (when present — hamburger uses this first)
            if full_val and not _re.search(r'[a-zA-Z]{3,}', full_val):
                failures.append(
                    f"'{group_label.strip()}' child full: is emoji-only: '{full_val}'"
                    " (hamburger renders full: first)"
                )

    if failures:
        for f in failures:
            report.fail(f"check_toolbar_child_label_text: {f}")
    else:
        report.ok("toolbar.js dropdown children — text: and full: labels all have word content")


def check_toolbar_lounges_in_flights(report: "Report") -> None:
    """HARD-FAIL if Lounges pages are not children of the Flights group in toolbar.js.

    Lounges moved into the ✈️ Flights dropdown on 2026-07-21. Two invariants:
    1. No top-level group named '💻 Lounges' (Lounges must NOT be standalone).
    2. Lounges-US.html and Lounges-Europe.html must appear as children of the
       ✈️ Flights group.

    Cleanliness Checks Rule 580. Added 2026-07-21.
    """
    if not TOOLBAR_JS.exists():
        report.fail("check_toolbar_lounges_in_flights: toolbar.js not found")
        return
    src = TOOLBAR_JS.read_text(encoding="utf-8")

    problems: list[str] = []

    # 1. No standalone Lounges group at the top level
    if re.search(r"group\s*:\s*['\"]💻\s*Lounges['\"]", src):
        problems.append(
            "toolbar.js still has a top-level group named '💻 Lounges' — "
            "Lounges must be children of the ✈️ Flights group, not a standalone group "
            "(Cleanliness Checks Rule 580)"
        )

    # 2. Lounges-US.html must appear inside the Flights group block
    # Strategy: find the Flights group block and check that both lounge files are in it
    flights_match = re.search(
        r"group\s*:\s*['\"]✈️\s*Flights['\"].*?(?=\{[^}]*group\s*:|$)",
        src,
        re.DOTALL,
    )
    if flights_match:
        flights_block = flights_match.group(0)
        if "Lounges-US.html" not in flights_block:
            problems.append(
                "Lounges-US.html is not a child of the ✈️ Flights group in toolbar.js "
                "(Cleanliness Checks Rule 580)"
            )
        if "Lounges-Europe.html" not in flights_block:
            problems.append(
                "Lounges-Europe.html is not a child of the ✈️ Flights group in toolbar.js "
                "(Cleanliness Checks Rule 580)"
            )
    else:
        # No Flights group found at all
        if "Lounges-US.html" not in src:
            problems.append(
                "Lounges-US.html not found anywhere in toolbar.js — must be a child of ✈️ Flights "
                "(Cleanliness Checks Rule 580)"
            )
        if "Lounges-Europe.html" not in src:
            problems.append(
                "Lounges-Europe.html not found anywhere in toolbar.js — must be a child of ✈️ Flights "
                "(Cleanliness Checks Rule 580)"
            )

    if problems:
        report.fail("check_toolbar_lounges_in_flights: " + " | ".join(problems))
    else:
        report.ok("toolbar.js Lounges pages — correctly nested as children of ✈️ Flights group")


def check_toolbar_flights_before_trains(report: "Report") -> None:
    """HARD-FAIL if the Trains entry appears before the Flights group in toolbar.js ITEMS.

    ✈️ Flights group must come before the European-Train-Guide.html plain link in the
    ITEMS array (character-position check on the source text).

    Cleanliness Checks Rule 581. Added 2026-07-21.
    """
    if not TOOLBAR_JS.exists():
        report.fail("check_toolbar_flights_before_trains: toolbar.js not found")
        return
    src = TOOLBAR_JS.read_text(encoding="utf-8")

    flights_match = re.search(r"group\s*:\s*['\"]✈️\s*Flights['\"]", src)
    trains_match = re.search(r"European-Train-Guide\.html", src)

    if not flights_match:
        report.fail(
            "check_toolbar_flights_before_trains: ✈️ Flights group not found in toolbar.js ITEMS "
            "(Cleanliness Checks Rule 581)"
        )
        return
    if not trains_match:
        report.fail(
            "check_toolbar_flights_before_trains: European-Train-Guide.html not found in toolbar.js "
            "(Cleanliness Checks Rule 581)"
        )
        return

    if trains_match.start() < flights_match.start():
        report.fail(
            "check_toolbar_flights_before_trains: European-Train-Guide.html appears before the "
            "✈️ Flights group in toolbar.js ITEMS — Flights must come first "
            "(Cleanliness Checks Rule 581)"
        )
    else:
        report.ok("toolbar.js ITEMS order — ✈️ Flights group appears before European Train link")


def check_toolbar_font_size_unified(report: "Report") -> None:
    """HARD-FAIL if toolbar font sizes drift from the unified 14px standard.

    On 2026-07-21 all toolbar font sizes were unified at 14px:
    - .tb-menu a must use font-size:14px (was 13px)

    Note: .tb-inner overflow-x:hidden was removed 2026-07-21 (created a BFC
    that clipped page content). .tb-links uses width:100%+space-between to
    fill the toolbar — no overflow occurs, no overflow-x property needed.

    Uses whitespace-collapsed source for tolerant matching.

    Cleanliness Checks Rule 582. Added 2026-07-21. Updated 2026-07-21.
    """
    if not TOOLBAR_JS.exists():
        report.fail("check_toolbar_font_size_unified: toolbar.js not found")
        return
    src = TOOLBAR_JS.read_text(encoding="utf-8")
    flat = re.sub(r"\s+", "", src)
    ...
    # .tb-menu a must have font-size:14px (not 13px)
    if ".tb-menua{" in flat or ".tb-menua," in flat:
        # Check that the 14px value appears near the .tb-menu a selector
        menu_a_match = re.search(r"\.tb-menu\s*a\s*\{([^}]*)\}", src)
        if menu_a_match:
            block = re.sub(r"\s+", "", menu_a_match.group(1))
            if "font-size:14px" not in block:
                if "font-size:13px" in block:
                    problems.append(
                        ".tb-menu a still uses font-size:13px — must be 14px to match .tb a "
                        "and .tb-ddbtn (Cleanliness Checks Rule 582)"
                    )
                else:
                    problems.append(
                        ".tb-menu a does not declare font-size:14px — unified toolbar font size "
                        "requires 14px (Cleanliness Checks Rule 582)"
                    )
    else:
        # Try flat search as fallback
        if ".tb-menua{font-size:13px" in flat or ".tb-menua{" in flat:
            if "font-size:14px" not in flat:
                problems.append(
                    ".tb-menu a font-size not found as 14px in toolbar.js "
                    "(Cleanliness Checks Rule 582)"
                )

    # Guard: .tb-inner must NOT revert to overflow-x:auto (the old scroll approach)
    inner_match = re.search(r"\.tb-inner\s*\{([^}]*)\}", src)
    if inner_match:
        inner_block = re.sub(r"\s+", "", inner_match.group(1))
        if "overflow-x:auto" in inner_block:
            problems.append(
                ".tb-inner uses overflow-x:auto — reverts to the old sliding toolbar. "
                "Use width:100%+space-between on .tb-links instead (Cleanliness Checks Rule 582)"
            )
        # Guard: .tb-inner must keep padding-right:20px so Visas tab doesn't touch right edge
        if "padding-right:20px" not in inner_block:
            problems.append(
                ".tb-inner is missing padding-right:20px — Visas tab will be flush against "
                "the right viewport edge (Cleanliness Checks Rule 582, added 2026-07-21)"
            )

    # Guard: .tb-site-title must be centered in its 220px zone (width:220px, text-align:center, margin-left:0)
    title_match = re.search(r"\.tb-site-title\s*(?:,\.tb\s+a\.tb-site-title[^{]*)?\{([^}]*)\}", src)
    if title_match:
        title_block = re.sub(r"\s+", "", title_match.group(1))
        if "width:220px" not in title_block:
            problems.append(
                ".tb-site-title missing width:220px — site title zone will not be centered "
                "between left edge and Guides tab (Cleanliness Checks Rule 582, added 2026-07-21)"
            )
        if "text-align:center" not in title_block:
            problems.append(
                ".tb-site-title missing text-align:center — title text will left-align "
                "instead of centering in its zone (Cleanliness Checks Rule 582, added 2026-07-21)"
            )
        if "margin-left:0" not in title_block:
            problems.append(
                ".tb-site-title has margin-left != 0 — off-center title zone "
                "(Cleanliness Checks Rule 582, added 2026-07-21)"
            )

    # Guard: mobile title (.tb-ham-label) must fill remaining bar space and be centered
    # (flex:1 gives it all space to the left of the MENU button; text-align:center centers
    # the text equidistant from left edge and button — parallel to .tb-site-title on desktop)
    # Find the ACTIVE .tb-ham-label rule (the one with display:block, inside the @media block)
    # — skip the default-hidden rule (.tb-ham-label{display:none})
    ham_label_match = None
    for m in re.finditer(r"\.tb-ham-label\{([^}]*)\}", flat):
        if "display:none" not in m.group(1):
            ham_label_match = m
            break
    if ham_label_match:
        ham_block = ham_label_match.group(1)
        if "flex:1" not in ham_block:
            problems.append(
                ".tb-ham-label missing flex:1 — mobile title will not fill space to the left "
                "of the MENU button, so centering breaks (Rule 582, added 2026-07-22)"
            )
        if "text-align:center" not in ham_block:
            problems.append(
                ".tb-ham-label missing text-align:center — mobile title will be left-aligned "
                "instead of centered between left edge and MENU button (Rule 582, added 2026-07-22)"
            )
        if "padding-left:" in ham_block and "padding-left:0" not in ham_block and "padding:0" not in ham_block:
            problems.append(
    ...
    breakpoints = re.findall(r"max-width:(\d+)px", flat)
    tb_breakpoints = [int(b) for b in breakpoints if int(b) > 800]  # ignore small utility breakpoints
    for bp in tb_breakpoints:
        if bp > 1260:
            problems.append(
                f"toolbar hamburger breakpoint max-width:{bp}px exceeds 1260px — "
                "desktop nav bar will hide on MacBook Air 13\" (1280px CSS viewport). "
                "Keep breakpoint ≤1260px (Cleanliness Checks Rule 582, added 2026-07-21)"
            )
            break

    if problems:
        report.fail("check_toolbar_font_size_unified: " + " | ".join(problems))
    else:
        report.ok("toolbar.js: fonts 14px, site-title centered (220px zone), "
                  "tb-ham-label centered (flex:1), tb-inner padding-right:20px, "
                  "breakpoint ≤1260px, no overflow-x:auto")


def check_non_guide_data_updated(report: "Report") -> None:
    """WARN when a non-guide content page with a toolbar-mount is missing
    data-updated="YYYY-MM". Every content page (Trip-Essentials, Best-Of, maps,
    Guides-Index) must carry data-updated so routines can record when content was
    last reviewed. Companion pages (read-about, stops-map inside Guides/) and
    Recommendations/ reports are excluded. Added 2026-07-22."""
    import re as _re
    if not WEB_ROOT.exists():
        return
    MOUNT_RE = _re.compile(r'<div\s+id="toolbar-mount"[^>]*>')
    UPDATED_RE = _re.compile(r'data-updated="(20\d{2}-(?:0[1-9]|1[0-2]))"')
    missing: list[str] = []
    checked = 0
    for fp in sorted(WEB_ROOT.rglob("*.html")):
        # Skip guide HTML, companion pages (inside Guides/), and Recommendations reports
        parts = fp.parts
        if any(p == "Guides" for p in parts):
            continue
        if any(p == "Recommendations" for p in parts):
            continue
        try:
            txt = fp.read_text(encoding="utf-8", errors="replace")
        except Exception:
            continue
        mount_m = MOUNT_RE.search(txt)
        if not mount_m:
            continue
        checked += 1
        if not UPDATED_RE.search(mount_m.group(0)):
            missing.append(_display_path(fp))
    if missing:
        report.warn(
            f"check_non_guide_data_updated: {len(missing)} non-guide page(s) missing "
            f"data-updated=\"YYYY-MM\" on toolbar-mount — add it (content-last-reviewed "
            f"month, Brain/Reference/Toolbar.html §10): {' · '.join(missing)}"
        )
    elif checked:
        report.ok(
            f"non-guide data-updated — all {checked} non-guide page(s) carry data-updated."
        )


def check_updated_stamp_position(report: "Report") -> None:
    """HARD-FAIL if toolbar.js appends the Updated stamp INSIDE #also-on-this-site.

    The stamp must always be inserted as a sibling AFTER #also-on-this-site (or
    #nearby-guides), never as a child via appendChild(). When it's a child it
    becomes a grid cell inside the pill grid and renders as a 'last pill', which
    is the bug this check prevents.

    Positive pattern (correct): also.parentNode.insertBefore(upd, also.nextSibling)
    Negative pattern (broken):  also.appendChild(upd)

    Added 2026-07-19.
    """
    import re as _re
    tb_path = ASSETS_DIR / 'toolbar.js'
    if not tb_path.exists():
        report.fail("check_updated_stamp_position: toolbar.js not found")
        return
    src = tb_path.read_text(encoding='utf-8', errors='replace')
    # Detect the broken pattern: also.appendChild(upd) inside repositionUpdatedStamp
    BROKEN = _re.compile(r'repositionUpdatedStamp[\s\S]{0,800}also\.appendChild\s*\(\s*upd\s*\)')
    if BROKEN.search(src):
        report.fail(
            "check_updated_stamp_position: repositionUpdatedStamp() uses also.appendChild(upd) — "
            "this lands the stamp INSIDE the #also-on-this-site pill grid (renders as a 'last pill'). "
            "Fix: also.parentNode.insertBefore(upd, also.nextSibling)"
        )
        return
    # Confirm the correct sibling-insert pattern is present
    CORRECT = _re.compile(r'also\.parentNode\.insertBefore\s*\(\s*upd\s*,\s*also\.nextSibling\s*\)')
    if not CORRECT.search(src):
        report.fail(
            "check_updated_stamp_position: repositionUpdatedStamp() does not use "
            "also.parentNode.insertBefore(upd, also.nextSibling) — "
            "stamp may not be placed correctly when #nearby-guides is absent"
        )
        return
    report.ok("toolbar.js repositionUpdatedStamp — stamp inserted as sibling after section, never inside pill grid")


def check_visited_pill_style(report: "Report") -> None:
    """HARD-FAIL if toolbar.js _injectVisitedPill() drifts from the locked spec.

    Locked spec (2026-07-19):
      - Anchor:        document.getElementById('nearby-guides')
      - Placement:     insertAdjacentElement('afterend', wrap)
      - Border:        border:1px solid #c8a44a
      - Text color:    color:#8a6c1a
      - Border-radius: border-radius:6px
      - Text content:  'Visited'
    ...
    tb_path = ASSETS_DIR / "toolbar.js"
    if not tb_path.exists():
        report.fail("check_visited_pill_style: toolbar.js not found")
        return
    src = tb_path.read_text(encoding="utf-8", errors="replace")

    # Extract the _injectVisitedPill function body using brace counting (not regex,
    # because the body contains inner function() literals that fool a greedy/lazy pattern).
    fn_start = src.find("function _injectVisitedPill()")
    if fn_start < 0:
        fn_start = src.find("function _injectVisitedPill ()")
    if fn_start < 0:
        report.fail(
            "check_visited_pill_style: _injectVisitedPill() not found in toolbar.js — "
            "the Visited pill will never appear on guide pages"
        )
        return
    brace_start = src.find("{", fn_start)
    depth, idx = 1, brace_start + 1
    while depth > 0 and idx < len(src):
        if src[idx] == "{":
            ...
            depth -= 1
        idx += 1
    body = src[fn_start:idx]

    failures = []
    # Anchor: must look up #nearby-guides
    if "getElementById('nearby-guides')" not in body and 'getElementById("nearby-guides")' not in body:
        failures.append("anchor must use getElementById('nearby-guides'), not a different selector")
    # Placement: insertAdjacentElement afterend
    if "insertAdjacentElement('afterend'" not in body and 'insertAdjacentElement("afterend"' not in body:
        failures.append("placement must use insertAdjacentElement('afterend', wrap)")
    # Border color
    if "border:1px solid #c8a44a" not in body:
        failures.append("pill border must be 'border:1px solid #c8a44a' (gold outline)")
    # Text color
    if "color:#8a6c1a" not in body:
        failures.append("pill text color must be 'color:#8a6c1a' (gold text)")
    # Border-radius
    if "border-radius:6px" not in body:
        failures.append("pill shape must be 'border-radius:6px' (matches also-on-this-site pills)")
    # Text content
    if "'Visited'" not in body and '"Visited"' not in body:
        failures.append("pill textContent must be 'Visited'")
    # Pill id
    if "tve-visited-pill" not in body:
        failures.append("pill id must be 'tve-visited-pill'")

    if failures:
        for f in failures:
            report.fail(f"check_visited_pill_style: {f}")
    else:
        report.ok("toolbar.js _injectVisitedPill — spec locked ✓ (gold border #c8a44a, #nearby-guides anchor, afterend placement, Visited text)")


def check_new_badge_css(report: "Report") -> None:
    """HARD-FAIL if guides-index-style.css .dest-new-badge drifts from the locked spec.

    Locked spec (2026-07-19 — amber honey badge on dest-cards, 14-day expiry):
    ...
    Added 2026-07-19.
    """
    css_path = ASSETS_DIR / "guides-index-style.css"
    if not css_path.exists():
        report.fail("check_new_badge_css: guides-index-style.css not found")
        return
    css = css_path.read_text(encoding="utf-8", errors="replace")

    # Extract .dest-new-badge block
    import re as _re
    m = _re.search(r'\.dest-new-badge\s*\{([^}]*)\}', css, _re.DOTALL)
    if not m:
        report.fail(
            "check_new_badge_css: .dest-new-badge rule missing from guides-index-style.css — "
            "the 'new' badge will never render on newly shipped cards"
        )
        return
    block = m.group(1)

    failures = []
    if "1.5px solid #e8c97a" not in block:
        failures.append("border must be '1.5px solid #e8c97a' (amber honey)")
    if "#fdecc8" not in block:
        failures.append("background must be '#fdecc8' (amber honey fill)")
    if "#7a4d00" not in block:
        failures.append("color must be '#7a4d00' (amber honey text)")
    if "border-radius: 3px" not in block and "border-radius:3px" not in block:
        failures.append("border-radius must be 3px")

    if failures:
        for f in failures:
            report.fail(f"check_new_badge_css: {f}")
    else:
        report.ok("guides-index-style.css .dest-new-badge — amber honey spec locked ✓")


def check_new_badge_js(report: "Report") -> None:
    """HARD-FAIL if Guides-Index.html is missing the 'new' badge injection JS.

    The badge JS must:
      - Read data-shipped from each dest-card
      - Check within 14 days
      - Inject a .dest-new-badge span

    Added 2026-07-19.
    """
    index_path = WEB_ROOT / "Guides-Index.html"
    if not index_path.exists():
        report.fail("check_new_badge_js: Guides-Index.html not found")
        return
    html = index_path.read_text(encoding="utf-8", errors="replace")

    failures = []
    if "data-shipped" not in html:
        failures.append("no data-shipped attribute or JS reference found")
    if "dest-new-badge" not in html:
        failures.append(".dest-new-badge class not referenced in badge JS")
    if "14 * DAY" not in html and "14 * 86400000" not in html and "14*DAY" not in html:
        failures.append("14-day expiry check missing (must use '14 * DAY' or equivalent)")

    if failures:
        for f in failures:
            report.fail(f"check_new_badge_js: {f}")
    else:
        report.ok("Guides-Index.html new-badge JS — data-shipped check + 14-day expiry + .dest-new-badge inject ✓")
...
def check_mobile_design_system(report: "Report") -> None:
    ...
    if gindex_p.exists():
        ...
        checks.append(('assets/guides-index-style.css', gindex_p, [
            ...
            ('mobile filter grid tile (.topbar .disc-btn) = 46px min-height / 0 radius (Form B — matches .fchip/.tchip/.lchip)',
             _re.compile(r'\.topbar \.disc-btn \{[^}]*min-height: 46px', _re.I)),
        ]))
    ...
    if guide_style_p.exists():
        ...
        checks.append(('assets/guide-style.css', guide_style_p, [
            ...
            ('extras-nav desktop gap = 3px (locked 2026-07-19 — nearly-touching pills so Train Stations fits row 1)',
             _re.compile(r'\.overview-extras \{[^}]*gap: 3px', _re.I)),
            ...
            ('also-on-this-site mobile: lone last pill spans full row (grid-column: span 6)',
             _re.compile(r'\.also-on-this-site-pill:last-child:nth-child\(3n\+1\) \{ grid-column: span 6', _re.I)),
            ('also-on-this-site mobile: 2-pill last row each spans half (span 3)',
             _re.compile(r'\.also-on-this-site-pill:last-child:nth-child\(3n\+2\) \{ grid-column: span 3', _re.I)),
        ]))
...
def check_no_beige_pill_button(report: "Report") -> None:
    ...
    # Catches both the literal hex and the CSS variable aliases --warm / --c-warm-bg.
    _WARM_VAL = r"(?:#fdf8f0|var\(\s*--(?:warm|c-warm-bg)\s*\))"
    WARM_BG = re.compile(r"background(?:-color)?\s*:\s*" + _WARM_VAL, re.I)
    # Inline pill/button element carrying a warm-token background.
    INLINE_WARM_ELEM = re.compile(
        r"<(?:a|button)\b[^>]*style=\"[^\"]*background(?:-color)?\s*:\s*" + _WARM_VAL, re.I
    )
    PILL_SEL = re.compile(r"pill|btn|button|chip|cta", re.I)
    STYLE_BLOCK = re.compile(r"<style[^>]*>(.*?)</style>", re.S | re.I)
    CSS_RULE = re.compile(r"([^{}]+)\{([^{}]*)\}", re.S)
    # Exemptions for warm-background on pill-named selectors:
    #  .stat-pill — informational badge (not an action/nav pill); warm background is intentional
    WARM_PILL_EXEMPT = re.compile(r"\.stat-pill", re.I)
    ...
    def scan_css(css_text: str, label: str):
        ...
        for rule in CSS_RULE.finditer(clean):
            ...
            if WARM_PILL_EXEMPT.search(selector):
                continue
            if WARM_BG.search(body) and PILL_SEL.search(selector):
                warm_match = WARM_BG.search(body)
                warm_val = warm_match.group(0) if warm_match else "#fdf8f0/var(--warm)"
                fails.append(
                    f"{label} — selector '{selector}' fills a pill/button with the warm "
                    f"card token ({warm_val}). Pills/buttons are white #ffffff — warm tokens are for cards only."
                )
    ...
    for html_path in sorted(html_pages):
        ...
        if INLINE_WARM_ELEM.search(text):
            fails.append(
                f"{rel} — an inline <a>/<button> fills with the warm card token (#fdf8f0/var(--warm)). "
                f"Pills/buttons are white #ffffff — warm tokens are for cards only."
            )
            ...
            for rule in CSS_RULE.finditer(style_match.group(1)):
                ...
                if WARM_BG.search(body) and PILL_SEL.search(selector):
                    ...
                    fails.append(
                        ...
                        f"card token ({warm_val}). Pills/buttons are white — warm tokens are for cards only."
                    )
...
def check_guide_pill_css_canonical(report: "Report") -> None:
    """HARD-FAIL if guide-style.css loses any locked canonical value for the
    four guide-page pill classes.

    POSITIVE ASSERTION — existing pill checks only BAN wrong values
    (check_pill_hover_behavior, check_active_pill_color, check_no_beige_pill_button).
    This check asserts the CORRECT values ARE present.  Catches:
      - deletion of a whole rule block (the selector disappears)
      - value drift to a valid-looking-but-wrong token (gold border changed to
        a different shade, gradient start/stop colours shifted)

    ── REST STATE (always visible) ───────────────────────────────────────────
    ...
    css_path = TRAVEL_ROOT / "Travel-Website" / "assets" / "guide-style.css"
    if not css_path.exists():
        report.fail("check_guide_pill_css_canonical: guide-style.css not found")
        return

    css = css_path.read_text(encoding="utf-8", errors="replace")

    # Parse all CSS rule blocks.  The flat regex handles nested @media wrappers
    # correctly: after finditer advances past the @media opening '{', the inner
    # rule's selector text (preceded by whitespace) is captured as group(1), and
    # the inner properties as group(2).  Multiple blocks with the same stripped
    # selector (unusual but safe) have their bodies merged so all properties are
    # visible to the invariant checks regardless of which block they live in.
    _CSS_RULE_RE = _re.compile(r"([^{}]+)\{([^{}]*)\}", _re.S)
    rules: dict = {}
    for _m in _CSS_RULE_RE.finditer(css):
        _sel = _m.group(1).strip()
        _body = _m.group(2)
        rules[_sel] = rules.get(_sel, "") + "\n" + _body

    fails = []

    def _assert(selector: str, prop: str, desc: str) -> None:
        body = rules.get(selector, "")
        if prop not in body:
            fails.append(
                f"selector {selector!r} is missing {prop!r} ({desc}) — "
                f"check guide-style.css for deletion or value drift"
            )

    # ── REST STATE ────────────────────────────────────────────────────────────
    _OEL  = ".overview-extras .overview-extra-link"   # chip-scoped (not bare class)
    _AOSP = ".also-on-this-site-pill"
    _OD   = ".overview-day"
    _NGP  = ".nearby-guide-pill"

    _assert(_OEL,  "background: var(--c-card-bg)",  "rest — warm card background")
    _assert(_OEL,  "border: 0.5px solid #c8a44a",   "rest — gold border")
...
def check_orphaned_task_folders(report: "Report") -> None:
    """Warn when ~/.claude/scheduled-tasks/ contains folders with no matching live cron.

    An orphaned folder is one whose name doesn't match any taskId returned by the
    scheduled-tasks SKILL.md registry.  Editing an orphan's SKILL.md is a silent
    no-op — the change never runs.  Added 2026-07-15 after a 62-folder / 29-live
    divergence was found (33 orphans).
    """
    import json as _json
    import subprocess as _sp
    tasks_dir = Path.home() / ".claude" / "scheduled-tasks"
    if not tasks_dir.is_dir():
        report.passes.append("orphaned-task-folders: scheduled-tasks dir not found — skip")
        return
    folders = {p.name for p in tasks_dir.iterdir() if p.is_dir()}
    if not folders:
        report.passes.append("orphaned-task-folders: no folders")
        return
    # Read the live task IDs from the list file written by guide_tools / the harness.
    # Fall back to checking for a SKILL.md that has a matching cron marker.
    live_ids: set[str] = set()
    live_ids_file = tasks_dir.parent / "scheduled-tasks.json"
    if live_ids_file.exists():
        try:
            data = _json.loads(live_ids_file.read_text())
            if isinstance(data, list):
                live_ids = {str(t.get("taskId", "")) for t in data if t.get("taskId")}
        except Exception:
            pass
    if not live_ids:
        # Heuristic fallback: a live folder's SKILL.md contains a cronExpression line
        # or an "enabled: true" marker written by the harness when it registers the cron.
        for folder in folders:
            md = tasks_dir / folder / "SKILL.md"
            if md.exists():
                text = md.read_text(errors="replace")
                if "cronExpression" in text or "enabled: true" in text:
                    live_ids.add(folder)
    if not live_ids:
        report.passes.append("orphaned-task-folders: cannot determine live IDs — skip")
        return
    orphans = sorted(folders - live_ids)
    if not orphans:
        report.passes.append(f"orphaned-task-folders: all {len(folders)} folders are live")
        return
    report.warnings.append(
        f"orphaned-task-folders: {len(orphans)} folder(s) have no live cron "
        f"(editing them is a silent no-op) — delete or archive: "
        + ", ".join(orphans[:8]) + (f" … +{len(orphans)-8} more" if len(orphans) > 8 else "")
    )


def check_train_time_dep_arr_color(report: "Report") -> None:
    """HARD-FAIL if guide-style.css gives .dep or .arr a color: property inside train-time.

    The departure/arrival time spans in timetable rows must NOT be specially colored —
    they inherit normal text color from .train-time. Any `color:` rule on
    `div.train .train-time .dep` or `.arr` makes the time numbers render in a
    distinctly golden-brown tone that looks like active/selected state drift.
    The locked standard: NO color: property on .dep / .arr within .train-time.
    (added 2026-07-16 after Santa Fe shipped with colored time numbers)
    """
    css_path = TRAVEL_ROOT / "Travel-Website" / "assets" / "guide-style.css"
    if not css_path.exists():
        report.warn("check_train_time_dep_arr_color: guide-style.css not found")
        return
    css = css_path.read_text(encoding="utf-8", errors="replace")
    ...
    # Find any rule block that selects .dep or .arr inside .train-time and sets color:
    pattern = re.compile(
        r'((?:div\.train\s+\.train-time\s+\.dep|div\.train\s+\.train-time\s+\.arr'
        r'|\.train-time\s+\.dep|\.train-time\s+\.arr)[^{]*)\{([^}]+)\}',
        re.S
    )
    for m in pattern.finditer(css_no_comments):
        ...
        if re.search(r'\bcolor\s*:', block):
            report.fail(
                f"check_train_time_dep_arr_color: HARD-FAIL — guide-style.css selector "
                f"'{selector}' sets color: on .dep/.arr inside .train-time. "
                "Departure/arrival time spans must NOT be specially colored — they inherit "
                "normal text color. Remove the color: declaration. "
                "(rule: timetable time numbers must not render in golden-brown/title color — added 2026-07-16)"
            )
            return
    report.passes.append(
        "check_train_time_dep_arr_color: .dep/.arr inside .train-time carry no color: override ✓"
    )


def check_train_time_left_padding(report: "Report") -> None:
    """HARD-FAIL if guide-style.css sets a left padding on .train-time rows.

    .train-time rows must sit flush with div.train's own 14px left padding —
    no additional left indent. The old value was padding: 2px 0 2px 16px;
    that 16px was designed to indent times under .train-header when the header
    lived INSIDE the box. The header is now a sibling ABOVE the box, so the
    16px just creates dead space before every departure time. Locked to:
    padding: 2px 0 (2-value shorthand, no left component). (added 2026-07-17)
    """
    css_path = TRAVEL_ROOT / "Travel-Website" / "assets" / "guide-style.css"
    if not css_path.exists():
        report.warn("check_train_time_left_padding: guide-style.css not found")
        return
    css = css_path.read_text(encoding="utf-8", errors="replace")
    css_no_comments = re.sub(r'/\*.*?\*/', '', css, flags=re.S)

    pattern = re.compile(r'(?:div\.train\s+)?\.train-time\s*\{([^}]+)\}', re.S | re.IGNORECASE)
    for m in pattern.finditer(css_no_comments):
        block = m.group(1)
        if re.search(r'\bpadding-left\s*:', block, re.IGNORECASE):
            report.fail(
                "check_train_time_left_padding: HARD-FAIL — guide-style.css sets padding-left "
                "on .train-time. Remove it — locked to padding: 2px 0 (no left indent). "
                "The 16px left was for when .train-header lived inside div.train; header is "
                "now a sibling above the box. (added 2026-07-17)"
            )
            return
        m2 = re.search(r'\bpadding\s*:\s*([\d.a-z%\s]+)', block, re.IGNORECASE)
        if m2:
            parts = m2.group(1).strip().split()
            if len(parts) >= 4 and parts[3] != '0':
                report.fail(
                    f"check_train_time_left_padding: HARD-FAIL — guide-style.css .train-time "
                    f"padding shorthand has a non-zero left value '{parts[3]}'. "
                    "Locked to: padding: 2px 0 (no left indent). (added 2026-07-17)"
                )
                return
    report.passes.append(
        "check_train_time_left_padding: .train-time has no left padding — times sit flush with box ✓"
    )


def check_station_box_first_child_margin(report: "Report") -> None:
    """HARD-FAIL if guide-style.css is missing the station-box first-child margin-top: 0 rule.

    station-box has padding-top: 0 and no border-top. Without an explicit
    margin-top: 0 on the first child, the first .stop-row's margin-top: 4px
    collapses THROUGH the container (CSS margin collapsing), appearing as a
    white gap between the extras-sub heading and the first station row. The
    fix mirrors .entry-body > div:first-child { margin-top: 0 } (line 670).
    (added 2026-07-16)
    """
    css_path = TRAVEL_ROOT / "Travel-Website" / "assets" / "guide-style.css"
    if not css_path.exists():
        report.warn("check_station_box_first_child_margin: guide-style.css not found")
        return
    css = css_path.read_text(encoding="utf-8", errors="replace")
    css_no_comments = re.sub(r'/\*.*?\*/', '', css, flags=re.S)

    # selector may be bare or ID-prefixed; block must set margin-top: 0
    has_rule = bool(re.search(
        r'station-box\s*>\s*\.stop-row\s*:first-child\b[^{]*\{[^}]*\bmargin-top\s*:\s*0\b',
        css_no_comments, re.S | re.IGNORECASE,
    ))
    if not has_rule:
        report.fail(
            "check_station_box_first_child_margin: HARD-FAIL — guide-style.css is missing "
            ".station-box > .stop-row:first-child { margin-top: 0 }. Without this rule the "
            "first stop-row's margin-top: 4px collapses through station-box (padding-top:0 + "
            "no border-top), creating a white gap between the extras-sub heading and the first "
            "station row. Add: #stations-near-hotel .station-box > .stop-row:first-child "
            "{ margin-top: 0; } (locked 2026-07-16)"
        )
        return
    report.passes.append(
        "check_station_box_first_child_margin: .station-box > .stop-row:first-child has margin-top: 0 ✓"
    )


def check_title_address_has_css_arrow(report: "Report") -> None:
    """HARD-FAIL if guide-style.css is missing the CSS-drawn terracotta arrow on .title-address a::after.

    The mobile hotel address card must show a terracotta chevron at the right edge
    to indicate the card is a tappable Google Maps link. The correct implementation
    is a CSS border-drawn arrow (border-top + border-right + rotate(45deg), color #b85c2a),
    NOT a Unicode ↗ character or Apple ↗️ emoji in the HTML (the Apple emoji renders blue
    and ignores CSS color). This is a mobile-only cue; the ::after rule lives inside
    the @media (max-width: 600px) block.

    Rule home: CLAUDE.md DriftyCat / Hotel Banner.html § 2b.
    """
    import re as _re
    css_path = TRAVEL_ROOT / "Travel-Website" / "assets" / "guide-style.css"
    if not css_path.is_file():
        report.warn("check_title_address_has_css_arrow: guide-style.css not found")
        return
    css = css_path.read_text(encoding="utf-8", errors="ignore")
    if not _re.search(
        r'\.title-address\s+a::after\s*\{[^}]*border-top\s*:',
        css,
    ):
        report.fail(
            "check_title_address_has_css_arrow: HARD-FAIL — guide-style.css is missing "
            ".title-address a::after with border-top (CSS-drawn terracotta chevron). "
            "The mobile hotel address card must have this arrow to indicate it is "
            "a tappable Google Maps link. Use a CSS border arrow (border-top: 1.5px solid #b85c2a; "
            "border-right: 1.5px solid #b85c2a; transform: rotate(45deg)) inside the "
            "@media (max-width:600px) block — NOT a Unicode ↗ or Apple ↗️ emoji in HTML. "
            "Hotel Banner.html § 2b / CLAUDE.md DriftyCat (added 2026-07-19)."
        )
        return
    report.passes.append(
        "check_title_address_has_css_arrow: .title-address a::after terracotta arrow present in guide-style.css ✓"
    )
...
def check_index_table_design(report: "Report") -> None:
    """HARD-FAIL if Currency-Guide or Plug-Adapter table drifts from the terracotta design.

    Both pages use an identical .index-table interaction pattern locked in 2026-07-15:
      - Outer border (not a drop box-shadow)
      - White rows, no zebra striping — no .idx-even / .idx-odd background rules
      - Terracotta box-shadow outline on hover (#b85c2a)
      - No filter:brightness() hover
      - Uppercase headers

    The monthly build_currency.py run historically clobbered these rules by regenerating
    the full file from a stale CSS template. This gate catches that before it ships.
    (added 2026-07-16)
    """
    pages = [
        TRAVEL_ROOT / "Travel-Website" / "Trip-Essentials" / "Currency-Guide.html",
        TRAVEL_ROOT / "Travel-Website" / "Trip-Essentials" / "Plug-Adapter" / "Plug-Adapter-Guide.html",
    ]
    for page in pages:
        if not page.exists():
            report.warn(f"check_index_table_design: {page.name} not found — skipped")
            continue
        html = page.read_text(encoding="utf-8", errors="replace")

        rel = page.relative_to(TRAVEL_ROOT)
        failures = []

        # Must have outer border (the new design), not just a drop box-shadow
        if re.search(r'\.index-table\s*\{[^}]*box-shadow\s*:\s*0\s+\d+px\s+\d+px', html):
            failures.append("uses drop box-shadow on .index-table instead of outer border — old design")

        # Must NOT have filter:brightness hover
        if re.search(r'\.index-table\s+tbody\s+tr:hover\s*\{\s*filter\s*:\s*brightness', html):
            failures.append("uses filter:brightness() on row hover — old design; must use terracotta box-shadow")

        # Must NOT have zebra-stripe rules (.idx-even / .idx-odd with background)
        if re.search(r'\.idx-even\s*\{[^}]*background', html):
            failures.append(".idx-even background rule present — zebra striping removed in current design")
        if re.search(r'\.idx-odd\s*\{[^}]*background', html):
            failures.append(".idx-odd background rule present — zebra striping removed in current design")

        # Must have terracotta hover via box-shadow on td
        if not re.search(r'\.index-table\s+tbody\s+tr:hover\s+td\s*\{[^}]*#b85c2a', html):
            failures.append("missing terracotta box-shadow (#b85c2a) on .index-table tbody tr:hover td")

        # Headers must be uppercase
        if not re.search(r'\.index-table\s+thead\s+th\s*\{[^}]*text-transform\s*:\s*uppercase', html):
            failures.append(".index-table thead th missing text-transform:uppercase")

        for f in failures:
            report.fail(f"check_index_table_design: {rel} — {f}")

        if not failures:
            report.passes.append(f"check_index_table_design: {rel.name} terracotta table design intact ✓")


def check_city_search_tvesearch(report: "Report") -> None:
    """HARD-FAIL if any page has a search input but no TVESearch wiring.
    Covers type=search inputs AND type=text inputs that act as search boxes
    (detected by placeholder containing 'city', 'country', 'state', or 'search').
    Every search box on the site must use the shared TVESearch typeahead dropdown.
    (added 2026-07-16; broadened to type=text search inputs 2026-07-17)"""
    import re as _re
    web_root = WEB_ROOT
    if not web_root.exists():
        report.warn("check_city_search_tvesearch: Travel-Website/ not found — skipped.")
        return
    SKIP_PARTS = {"Guides", "_build", "Recommendations"}
    pages = [
        p for p in web_root.rglob("*.html")
        if not any(part in SKIP_PARTS for part in p.parts)
    ]
    _type_search_re = _re.compile(r'type=["\']search["\']', _re.IGNORECASE)
    _text_search_re = _re.compile(
        r'type=["\']text["\'][^>]*placeholder=["\'][^"\']*(?:city|country|state|search)[^"\']*["\']'
        r'|placeholder=["\'][^"\']*(?:city|country|state|search)[^"\']*["\'][^>]*type=["\']text["\']',
        _re.IGNORECASE
    )
    for page in pages:
        html = page.read_text(encoding="utf-8", errors="replace")
        rel = page.relative_to(TRAVEL_ROOT)
        has_search = _type_search_re.search(html) or _text_search_re.search(html)
        if not has_search:
            continue
        if "TVESearch" not in html and "search-autocomplete.js" not in html:
            report.fail(
                f"check_city_search_tvesearch: {rel} has a search input "
                f"but is not wired to TVESearch (missing search-autocomplete.js + TVESearch.attach) — "
                f"every search box must use the shared typeahead dropdown (added 2026-07-16)"
            )
        else:
            report.passes.append(f"check_city_search_tvesearch: {rel.name} TVESearch wired ✓")


def check_search_aria_labels(report: "Report") -> None:
    """WARN if any type=search input is missing a non-empty aria-label attribute.
    The 2026-07-17 aria-label sweep fixed 44 inputs but gaps can reappear as new
    pages are added. This gate prevents silent accessibility regressions.
    Scope: all Travel-Website/**/*.html except Guides/, _build/, Recommendations/.
    (added 2026-07-19)"""
    import re as _re
    web_root = WEB_ROOT
    if not web_root.exists():
        report.warn("check_search_aria_labels: Travel-Website/ not found — skipped.")
        return
    SKIP_PARTS = {"Guides", "_build", "Recommendations"}
    pages = [
        p for p in web_root.rglob("*.html")
    ...
    _input_re = _re.compile(r'<input\b([^>]*?)/?>', _re.IGNORECASE | _re.DOTALL)
    _type_search_re = _re.compile(r'\btype=["\']search["\']', _re.IGNORECASE)
    _aria_label_re = _re.compile(r'\baria-label=["\'][^"\']+["\']', _re.IGNORECASE)
    missing = []
    for page in pages:
        text = page.read_text(encoding="utf-8", errors="replace")
        rel = page.relative_to(TRAVEL_ROOT)
        for m in _input_re.finditer(text):
            attrs = m.group(1)
            if _type_search_re.search(attrs) and not _aria_label_re.search(attrs):
                # Extract id for a useful message
                id_m = _re.search(r'\bid=["\']([^"\']+)["\']', attrs, _re.IGNORECASE)
                inp_id = id_m.group(1) if id_m else "(no id)"
                missing.append(f"{rel} #{inp_id}")
    if missing:
        for item in missing:
            report.warn(
                f"check_search_aria_labels: {item} has type=search but no aria-label "
                f"— screen readers announce it as an unlabeled field (added 2026-07-19)"
            )
    else:
        report.passes.append("check_search_aria_labels: all search inputs have aria-label ✓")


def check_sidebar_panel_background(report: "Report") -> None:
    """HARD-FAIL if guides-index-style.css re-adds a terracotta background to the
    Best-of or Also-on-this-site sidebar panels.

    Both .country[data-country="specialized-trips"] and
    .country[data-country="also-on-site"] must use the default var(--warm) beige —
    same as every other country panel. The terracotta override was removed 2026-07-19.
    Drift class: a crib CSS rewrite re-adds 'background:#b85c2a' or the gradient to
    these selectors, making the sidebar panels brown again.
    """
    css_path = TRAVEL_ROOT / "Travel-Website/assets/guides-index-style.css"
    if not css_path.exists():
        report.warn("check_sidebar_panel_background: guides-index-style.css not found")
        return
    css = css_path.read_text(encoding="utf-8")
    import re
    # Look for a background rule on either panel selector
    pattern = re.compile(
        r'\.country\[data-country=["\'](?:specialized-trips|also-on-site)["\'][^\{]*\{[^\}]*background\s*:[^;]*(?:#b85c|#7a3b|linear-gradient)[^;]*;',
        re.IGNORECASE | re.DOTALL
    )
    if pattern.search(css):
        report.fail(
            "check_sidebar_panel_background: guides-index-style.css has a terracotta "
            "background on [data-country=specialized-trips] or [data-country=also-on-site] — "
            "both panels must use default var(--warm) beige (terracotta override removed 2026-07-19). "
            "Delete the background rule from these selectors. (Rule 561)"
        )
    else:
        report.passes.append("check_sidebar_panel_background: sidebar panels use default beige ✓")


def check_ham_btn_style(report: "Report") -> None:
    """HARD-FAIL if toolbar.js hamburger button has drifted from the locked spec.

    Final spec (2026-07-19):
      Pill: background:#6e3117, border-radius:8px, width:82px, padding:11px 0,
            margin:0 14px 0 0, gap:8px, color:#fff
      MENU icon: SVG 18×13 with three filled white rects (fill="#fff"), height=2.5
      CLOSE icon: SVG 14×14 two white lines stroke="#fff" stroke-width="2.5"
      Label: font-size:12px, font-weight:700, color:#fff on the span
      Visibility: CSS-only — no display: in inline style
      mobile.css: must NOT have background:none on .tb-ham
    Rule 564 / Toolbar.html §3.
    """
    import re as _re
    tb_path = TRAVEL_ROOT / "Travel-Website/assets/toolbar.js"
    mobile_path = TRAVEL_ROOT / "Travel-Website/assets/mobile.css"
    if not tb_path.exists():
        report.warn("check_ham_btn_style: toolbar.js not found")
        return
    tb = tb_path.read_text(encoding="utf-8")

    m = _re.search(r"hamBtn\.style\.cssText\s*=\s*['\"]([^'\"]+)['\"]", tb)
    if not m:
        report.warn("check_ham_btn_style: could not locate hamBtn.style.cssText assignment")
        return
    css_text = m.group(1)

    errors = []
    if "background:#6e3117" not in css_text:
        errors.append("hamBtn pill background must be #6e3117 (dark terracotta)")
    if "width:82px" not in css_text:
        errors.append("hamBtn width must be 82px (locks MENU and CLOSE to same size)")
    if "display:" in css_text:
        errors.append("hamBtn inline style must NOT contain display: — CSS @media owns visibility")
    if "☰" in tb:
        errors.append("toolbar.js still contains Unicode ☰ glyph — use filled SVG rects")
    if 'fill="#fff"' not in tb and "fill='#fff'" not in tb and 'fill="white"' not in tb:
        errors.append("hamBtn SVG bars must use filled white rects (fill=\"#fff\")")
    if mobile_path.exists():
        mob = mobile_path.read_text(encoding="utf-8")
        ham_block_m = _re.search(r'\.tb-ham\s*\{([^}]+)\}', mob)
        if ham_block_m:
            ham_block = ham_block_m.group(1)
            if "background" in ham_block:
                errors.append("mobile.css .tb-ham block must NOT set background — it wipes the #6e3117 pill color")
            if "padding" in ham_block:
                errors.append("mobile.css .tb-ham block must NOT set padding — it overrides inline padding:11px 0 and squishes the button")

    if errors:
        report.fail(
            "check_ham_btn_style: mobile hamburger button has drifted from spec — "
            + "; ".join(errors)
            + ". Spec: background:#6e3117, border-radius:8px, width:82px, filled white SVG rects, no display: inline, no background:none in mobile.css. (Rule 564)"
        )
    else:
        report.passes.append("check_ham_btn_style: hamburger button matches dark terracotta pill spec ✓")


def check_ham_menu_height(report: "Report") -> None:
    """HARD-FAIL if toolbar.js .tb-ham-menu uses a forced height/max-height.

    Locked spec (2026-07-20):
      .tb-ham-menu: position:absolute; top:100%; NO height or max-height.
      The panel must be natural height (only as tall as its items) so it
      scrolls with the page and leaves no blank space below the last entry.
      A forced height/max-height makes it taller than its content, creating
      a white void below the last menu item (Rule 567).
    """
    if not TOOLBAR_JS.exists():
        report.warn("check_ham_menu_height: toolbar.js not found")
        return
    src = TOOLBAR_JS.read_text(encoding="utf-8")
    # Find the .tb-ham-menu CSS block and check it has no height: or max-height:
    import re
    m = re.search(r'\.tb-ham-menu\{[^}]+\}', src)
    if not m:
        report.warn("check_ham_menu_height: could not locate .tb-ham-menu CSS block in toolbar.js")
        return
    block = m.group(0)
    if "max-height:" in block or re.search(r'(?<!min-)height:', block):
        report.fail(
            "check_ham_menu_height: .tb-ham-menu CSS block contains a height or max-height "
            "constraint — the panel must be natural height (as tall as its items only). "
            "Remove height/max-height from the .tb-ham-menu block in toolbar.js (Rule 567)."
        )
    else:
        report.passes.append("check_ham_menu_height: .tb-ham-menu has no forced height — natural content height ✓")


def check_mobile_menu_sections(report: "Report") -> None:
    """HARD-FAIL if toolbar.js is missing the Best Of or Also on this site sections
    in the mobile hamburger menu.

    Added 2026-07-19: the mobile menu appends two extra sections after the main ITEMS
    loop — a 'Best Of' section listing all Best-of pages, and an 'Also on this site'
    section listing pages not in the main toolbar. Both are required so mobile users
    can reach these pages (the desktop sidebar is hidden on mobile).
    Drift class: a toolbar.js rewrite strips the two IIFE blocks at the end of the
    hamburger-menu build section.
    """
    tb_path = TRAVEL_ROOT / "Travel-Website/assets/toolbar.js"
    if not tb_path.exists():
        report.warn("check_mobile_menu_sections: toolbar.js not found")
        return
    tb = tb_path.read_text(encoding="utf-8")
    missing = []
    if "Best-Amusement-Parks.html" not in tb or "'Best Of'" not in tb and '"Best Of"' not in tb:
        missing.append("Best Of section")
    if "Budget-Guide.html" not in tb or "'Also on this site'" not in tb and '"Also on this site"' not in tb:
        missing.append("Also on this site section")
    if missing:
        report.fail(
            f"check_mobile_menu_sections: toolbar.js mobile hamburger menu is missing: "
            f"{', '.join(missing)} — both sections were added 2026-07-19 to give mobile "
            f"users access to Best-of pages and hub pages not in the main toolbar. "
            f"Re-add the IIFE blocks after the ITEMS.forEach loop. (Rule 562)"
        )
    else:
        report.passes.append("check_mobile_menu_sections: mobile menu has Best Of + Also on this site ✓")


def check_mobile_trips_injection(report: "Report") -> None:
    """HARD-FAIL if toolbar.js is missing the owner-directed My Trips injection
    in the mobile hamburger menu (added 2026-07-20, stripped by crib same day).

    The injection lives inside the ITEMS.forEach loop: after the Guides-Index link
    is appended, a second <a> pointing to Trip-Essentials/Trips.html is inserted
    with text '✈️ My Trips'. This is OWNER-DIRECTED and must never be removed.
    Drift class: toolbar.js rewrite drops the injection block.
    See Toolbar.html § 18b, Navigation.html § 8, Cleanliness Checks Rule 569.
    """
    tb_path = TRAVEL_ROOT / "Travel-Website/assets/toolbar.js"
    if not tb_path.exists():
        report.warn("check_mobile_trips_injection: toolbar.js not found")
        return
    tb = tb_path.read_text(encoding="utf-8")
    if "Trip-Essentials/Trips.html" not in tb or "My Trips" not in tb:
        report.fail(
            "check_mobile_trips_injection: toolbar.js is missing the owner-directed "
            "✈️ My Trips injection in the mobile hamburger menu (added 2026-07-20). "
            "Re-add the block inside ITEMS.forEach after the Guides-Index link: create "
            "an <a> with href=base+'Trip-Essentials/Trips.html' and text='✈️ My Trips', "
            "gated on /Guides-Index\\.html$/.test(item.href). "
            "See Toolbar.html § 18b + Cleanliness Checks Rule 569."
        )
    else:
        report.passes.append("check_mobile_trips_injection: ✈️ My Trips mobile injection present in toolbar.js ✓")


def check_no_ham_stats_injection(report: "Report") -> None:
    """HARD-FAIL if Guides-Index.html still injects a div.tb-ham-stats line
    into the mobile hamburger menu.

    Rule 579 (added 2026-07-20): The hamburger menu is navigation-only.
    A mini-stats line (e.g. "82 visited · 28 countries · 37% complete")
    was injected after the My Trips link by buildLSP() and removed on
    owner direction — stats belong on Personal-Stats.html, not the nav bar.
    The topbar pill update (lsp-tb-text / lsp-tb-fill) is fine and must stay;
    only the hamburger injection block must be absent.
    Drift class: buildLSP() in Guides-Index.html re-adds the hamburger
    injection block (mini.className = 'tb-ham-stats').
    """
    index_path = TRAVEL_ROOT / "Travel-Website" / "Guides-Index.html"
    if not index_path.exists():
        report.warn("check_no_ham_stats_injection: Guides-Index.html not found")
        return
    src = index_path.read_text(encoding="utf-8")
    if "tb-ham-stats" in src:
        report.fail(
            "check_no_ham_stats_injection: Guides-Index.html contains 'tb-ham-stats' — "
            "the hamburger mini-stats injection is banned (Rule 579). "
            "Remove the hamburger injection block from buildLSP() in Guides-Index.html. "
            "The topbar pill update (lsp-tb-text / lsp-tb-fill) must stay."
        )
    else:
        report.passes.append(
            "check_no_ham_stats_injection: no tb-ham-stats injection in Guides-Index.html ✓"
        )


def check_no_visited_in_hamburger(report: "Report") -> None:
    """HARD-FAIL if toolbar.js adds a '🟡 Visited' entry to the hamburger menu,
    or if Personal-Stats.html re-adds the #ps-visited-btn button.

    Rule 583 (added 2026-07-21): The Visited control lives ONLY on World-Map.html
    as the #visited-pill-mobile floating pill. The hamburger must have NO '🟡 Visited'
    entry (drift class b). Personal-Stats must have NO #ps-visited-btn (drift class a).
    Full spec: Brain/Reference/Maps-Layout.html § 12.
    """
    failures = []
    ...
    if tb_path.exists():
        ...
        if "🟡 Visited" in tb:
            failures.append(
                "toolbar.js contains '🟡 Visited' — Visited entry must NOT appear in the "
                "hamburger menu. The Visited control lives only on World-Map.html as "
                "#visited-pill-mobile. Full spec: Maps-Layout.html § 12."
            )
    else:
        report.warn("check_no_visited_in_hamburger: toolbar.js not found")
        return

    ps_path = TRAVEL_ROOT / "Travel-Website/Trip-Essentials/Personal-Stats.html"
    if ps_path.exists():
        ps = ps_path.read_text(encoding="utf-8")
        if "ps-visited-btn" in ps:
            failures.append(
                "Personal-Stats.html contains 'ps-visited-btn' — the old Visited button "
                "must not appear on Personal-Stats. Only a '🗺️ View Full Map' anchor link "
                "is allowed there (navigates to World-Map.html?from=stats). "
                "Full spec: Maps-Layout.html § 12."
            )

    if failures:
        for f in failures:
            report.fail(f"check_no_visited_in_hamburger: {f}")
    else:
        report.passes.append(
            "check_no_visited_in_hamburger: no '🟡 Visited' in hamburger, "
            "no ps-visited-btn in Personal-Stats ✓"
        )


def check_extras_title_gap(report: "Report") -> None:
    """HARD-FAIL if guide-style.css is missing the extras-section title gap fix.

    Two independent sources create the white gap between a section title's underline
    and the first colored .extras-sub entry:

    Source 1 — margin-top specificity battle:
      '#section-id .extras-sub { margin: 14px 0 0 }' has specificity (1,1,0) and
      beats the universal reset '.extras-section .extras-title + .extras-sub { margin-top: 0 }'
      (specificity 0,2,0). Fix: ID-scoped '#section-id .extras-title + .extras-sub
      { margin-top: 0 }' at specificity (1,2,0) wins.

    Source 2 — .extras-title margin-bottom:
      '.extras-title { margin-bottom: 16px }' always creates a 16px gap between the
      underline and the first entry regardless of the margin-top fix above.
      Fix: '.extras-section .extras-title:has(+ .extras-sub) { margin-bottom: 0 }'
      collapses it when the title is immediately followed by an .extras-sub.

    Both rules must be present or the gap comes back. This is the check that makes
    it fail at session start so the fix can't be silently reverted by a crib.
    (added 2026-07-19)"""
    import re as _re
    css_path = TRAVEL_ROOT / "Travel-Website" / "assets" / "guide-style.css"
    if not css_path.is_file():
        report.warn("check_extras_title_gap: guide-style.css not found — skipped")
        return
    css = css_path.read_text(encoding="utf-8", errors="ignore")

    # Source 2 fix: .extras-title:has(+ .extras-sub) { margin-bottom: 0 }
    if not _re.search(
        r'\.extras-title\s*:has\s*\(\s*\+\s*\.extras-sub\s*\)\s*\{[^}]*margin-bottom\s*:\s*0',
        css,
    ):
        report.fail(
            "check_extras_title_gap: HARD-FAIL — guide-style.css is missing "
            "'.extras-section .extras-title:has(+ .extras-sub) { margin-bottom: 0 }'. "
            "The 16px margin-bottom on .extras-title creates a visible white gap above the "
            "first colored entry even after the margin-top fix. This rule closes it. "
            "Enforced: brain_check check_extras_title_gap. (added 2026-07-19)"
        )
        return

    # Source 1 fix: ID-scoped overrides at (1,2,0)
    required_ids = [
        "#getting-around",
        "#heads-up",
        "#michelin",
    ...
    missing_ids = [
        ...
        if not _re.search(
            _re.escape(sid) + r'\s+\.extras-title\s*\+\s*\.extras-sub\s*\{\s*margin-top\s*:\s*0',
            css,
        )
    ]
    if missing_ids:
        report.fail(
            "check_extras_title_gap: HARD-FAIL — guide-style.css is missing ID-scoped "
            "'.extras-title + .extras-sub { margin-top: 0 }' overrides for: "
            + ", ".join(missing_ids) + ". "
            "Required because #id .extras-sub { margin: 14px 0 0 } (1,1,0) beats the "
            "universal reset (0,2,0); only an ID+2-class selector (1,2,0) wins. "
            "(added 2026-07-19)"
        )
        return

    report.passes.append(
        "check_extras_title_gap: extras-section title gap fix present "
        "(margin-bottom:0 :has-guard + margin-top:0 ID-scoped overrides) ✓"
    )
...
def main(argv: list[str]) -> int:
    ...
    check_world_map_render_cities_disabled(report)      # fails if renderCities() early return removed — guide-only pins invariant (Maps-Layout.html §10; Cleanliness Checks Rule 577; added 2026-07-20)
    check_world_map_load_regions_disabled(report)       # fails if maybeLoadRegions() early return removed — non-US/CA subdivisions must stay disabled (Maps-Layout.html §10; Cleanliness Checks Rule 578; added 2026-07-20)
    ...
    check_toolbar_child_label_text(report)              # fails if any toolbar dropdown group child has an emoji-only text: label (no word content) — renders as icon-only in mobile hamburger (added 2026-07-19)
    check_toolbar_lounges_in_flights(report)            # fails if Lounges-US/Europe.html are not children of ✈️ Flights group, or a standalone Lounges group exists (Rule 580, added 2026-07-21)
    check_toolbar_flights_before_trains(report)         # fails if European-Train-Guide.html appears before the ✈️ Flights group in ITEMS order (Rule 581, added 2026-07-21)
    check_toolbar_font_size_unified(report)             # Rule 582: 14px fonts, site-title 220px centered, tb-inner padding-right:20px, breakpoint ≤1260px, no overflow-x:auto
    check_non_guide_data_updated(report)                # warns if any non-guide content page is missing data-updated="YYYY-MM" on toolbar-mount (content-last-reviewed month; added 2026-07-22)
    ...
    check_guide_pill_css_canonical(report)              # HARD-FAIL if guide-style.css loses any locked canonical value for all 4 guide pill classes — positive assertion of rest+hover/active CSS (gold border #c8a44a, terracotta gradient fills, white text); complements the ban-based checks (added 2026-07-16)
    ...
    check_orphaned_task_folders(report)                 # warns on scheduled-task folders with no matching live cron — editing an orphan is a silent no-op (added 2026-07-15)
    check_train_time_dep_arr_color(report)              # HARD-FAILS if guide-style.css sets color: on .dep/.arr within .train-time — time numbers must not be specially colored (added 2026-07-16)
    check_train_time_left_padding(report)               # HARD-FAILS if guide-style.css sets a left padding on .train-time — locked to padding: 2px 0 (no left indent; added 2026-07-17)
    check_station_box_first_child_margin(report)        # HARD-FAILS if guide-style.css is missing .station-box > .stop-row:first-child { margin-top:0 } — without it the first stop-row's 4px margin collapses through the container (padding-top:0 + no border-top), creating a white gap (added 2026-07-16)
    check_title_address_has_css_arrow(report)            # HARD-FAILS if guide-style.css is missing .title-address a::after with border-top — mobile hotel address card must show a CSS-drawn terracotta chevron (not a Unicode ↗ or Apple ↗️ emoji); hotel address in HTML must have plain text only (added 2026-07-19, corrected 2026-07-19)
    check_index_table_design(report)                    # HARD-FAILS if Currency-Guide or Plug-Adapter drifts from terracotta box-shadow table design: no drop-shadow, no filter:brightness hover, no zebra .idx-even/.idx-odd, uppercase headers (added 2026-07-16)
    check_city_search_tvesearch(report)                 # HARD-FAILS if a page has 'City or country' search input but no TVESearch wiring (added 2026-07-16)
    check_search_aria_labels(report)                    # WARN if any type=search input is missing aria-label — prevents accessibility gap from 2026-07-17 sweep recurring (added 2026-07-19)
    check_extras_title_gap(report)                      # HARD-FAILS if guide-style.css loses either gap-fix rule: .extras-title:has(+.extras-sub){margin-bottom:0} (closes 16px title gap) OR ID-scoped .extras-title+.extras-sub{margin-top:0} overrides (closes 14px specificity gap); gap keeps recurring from crib CSS rewrites (added 2026-07-19)
    check_updated_stamp_position(report)               # HARD-FAIL if toolbar.js repositionUpdatedStamp() uses also.appendChild(upd) — stamp must be a sibling after #also-on-this-site, never a child/grid-cell inside it (added 2026-07-19)
    check_visited_pill_style(report)                   # HARD-FAIL if toolbar.js _injectVisitedPill() drifts from spec — border:#c8a44a, color:#8a6c1a, border-radius:6px, #nearby-guides anchor, afterend placement, 'Visited' text (added 2026-07-19)
    check_new_badge_css(report)                        # HARD-FAIL if guides-index-style.css .dest-new-badge loses amber honey spec — 1.5px solid #e8c97a border, #fdecc8 bg, #7a4d00 text, 3px radius (added 2026-07-19)
    check_new_badge_js(report)                         # HARD-FAIL if Guides-Index.html badge JS is missing data-shipped check, 14-day expiry, or .dest-new-badge inject (added 2026-07-19)
    check_sidebar_panel_background(report)             # HARD-FAIL if guides-index-style.css re-adds terracotta bg to specialized-trips or also-on-site sidebar panels — both must stay default beige var(--warm) (added 2026-07-19)
    check_mobile_menu_sections(report)                 # HARD-FAIL if toolbar.js mobile menu is missing the Best Of or Also on this site sections appended after the ITEMS loop (added 2026-07-19)
    check_mobile_trips_injection(report)               # HARD-FAIL if toolbar.js is missing the owner-directed ✈️ My Trips injection under Guides in the mobile hamburger menu (added 2026-07-20, Rule 569)
    check_ham_btn_style(report)                        # HARD-FAIL if toolbar.js hamburger button drifts from dark-terracotta-pill spec: background:#6e3117, border-radius:8px, no display: inline, SVG bars fill="#fff" (Rule 564, added 2026-07-19)
    check_ham_menu_height(report)                      # HARD-FAIL if toolbar.js .tb-ham-menu uses max-height instead of height — page bleeds through below menu when list is shorter than viewport (Rule 567, added 2026-07-20)
    check_no_ham_stats_injection(report)               # HARD-FAIL if Guides-Index.html still injects div.tb-ham-stats into the hamburger — mini-stats are banned; hamburger is navigation-only (Rule 579, added 2026-07-20)
    check_no_visited_in_hamburger(report)              # HARD-FAIL if toolbar.js adds '🟡 Visited' to the hamburger menu OR Personal-Stats.html re-adds #ps-visited-btn — Visited control lives ONLY on World-Map.html as #visited-pill-mobile (Rule 583, added 2026-07-21)
