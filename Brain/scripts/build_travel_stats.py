...
.hero-card {{ position: relative; background: var(--surface); border: 1px solid var(--border); border-radius: 10px; padding: 13px 12px 12px; text-align: center; overflow: hidden; }}
.hero-card::before {{ content: ""; position: absolute; top: 0; left: 0; right: 0; height: 3px; background: currentColor; opacity: .9; }}
.hero-icon {{ font-size: 16px; line-height: 1; margin-bottom: 5px; }}
.hero-num {{ font-size: 22px; font-weight: 800; letter-spacing: -0.03em; line-height: 1; margin-bottom: 3px; }}
...
.qf {{ background: var(--surface2); border: 1px solid var(--border2); border-radius: 9px; padding: 12px 14px; display: flex; flex-direction: column; gap: 2px; }}
...
/* ===== Section jump nav ===== */
.stats-nav {{ display: flex; flex-wrap: wrap; gap: 6px; margin: 0 0 18px; }}
.stats-nav a {{ font-size: var(--fs-sub,12px); color: #6a5230; text-decoration: none; padding: 4px 11px; border: 1px solid var(--border); border-radius: 20px; background: var(--surface2); white-space: nowrap; transition: all .15s; }}
.stats-nav a:hover {{ border-color: var(--accent); color: var(--accent); background: rgba(184,92,42,.08); }}
