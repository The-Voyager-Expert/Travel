...
def _guide_folder(path: str) -> str:
    """Folder name from an index href or search-index url.

    index.html cards point at "./Guides/{Folder}/{slug}_vN.html" and the search
    index stores "Guides/{Folder}/{slug}_vN.html". Both carry a leading Guides/
    segment, so a bare split("/")[0] returns the literal "Guides" for every guide
    — which silently made the folder comparisons below match nothing and reported
    the whole fleet as missing. (The prefix arrived with the single-index
...
def main() -> int:
    ...
    if sj.exists():
        try:
            for g in json.loads(_read(sj)).get("guides", []):
                if g.get("u"):
                    search_folders.add(_guide_folder(g["u"]))
                if g.get("c"):
    ...
    for href, name in re.findall(
        ...
        folder_to_name[_guide_folder(href)] = name.strip()
    ...
    for folder in guides:
        ...
        for f in {re.escape(folder), re.escape(quote(folder))}:
            ...
            if cm:
                ...
                break
        # index.html lives at WEB root and its card hrefs are "./Guides/{Folder}/…",
        # so they resolve against WEB — not GUIDES_DIR, which would look for
        # Guides/Guides/{Folder}/… and report every shipped guide as missing.
        if not card_href or not _link_ok(WEB, card_href):
